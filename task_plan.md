...
2026-09-09 Owner要求综合语言分支、过程表示、参数生成和监督/闭环不一致，尽可能通过分析与诊断实验寻找原因并提出解决方案。**禁止正式架构或训练方式修改，禁止启动正式训练。** 允许只读/CPU分析、冻结模型forward与临时接口干预，以及事先界定的非正式探索验证；保留科学信息墙、train24梯度边界及GPU/存储合同。当前400评测已完整结束，不恢复500/600或先前暂停的正式候选。

本轮完成标准：收齐现有400证据；用有区分力的实际实验逐步检验竞争解释；交付有原件支持的原因排序、排除边界、最早可定位接口与具体解决方案。不能承诺所有原因均可唯一识别，也不能以更多诊断数量代替信息增益。

1. 已完成上下文400正式证据收尾，validation90/400、train49/96。
2. 已完成实际监督覆盖与历史专家/clone/G1/G2审计，保留近等价反证。
3. 已完成B1/B2原生功能与动作面板、B3 480行冻结分支闭环。
4. 已完成八task×三接口固定64步临时求解、4096query新time/noise复核、192行接口闭环及四条目标状态回放/专家参照。
5. 已交付证据排序、局部定位边界及方案，详见`docs/horizon_k1_causal_diagnostics_20260909.md` §13；全部676行诊断闭环完整。正式修改和训练保持未启动，最终视频controls/Test封存，不恢复长期训练目标。

本轮完成；当前没有待执行实验。即时完成快照见progress，原件总索引为`causal_diagnostics_20260909/summary.json`。后续方案是讨论内容，不是active implementation或launch授权。
