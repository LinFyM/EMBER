...
**整体goal未完成。当前active仅为[冻结局部动作生成诊断](docs/frozen_local_action_decode_audit.md)。**
时间对齐与冻结正例复核均已按完整证据关闭。当前以固定旧local两臂step200检验纯视频动作还原的前提，
不恢复历史训练、不扩大旧闭环复核或运行最终controls。
...
可以从语言免费获得，也不是视频无用定理。当前新登记诊断只检验现成局部读出的动作知识，不创建或训练新模型；
纯噪声生成不能超过任务均值及frame_set时，停止把该现成动作读出直接接入参数生成的提案依据。
