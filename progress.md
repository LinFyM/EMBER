...
- 当前唯一GPU作业为第10节fresh训练：gpu01:0/4/5/6，world4，tmux `ember-teacher-vl`，200updates/800条件/51,200queries，保存100/200。两组读取Meta共同学习、source trainable=0，NCCL/NUMA与逻辑batch已由实际run contract确认。
- 启动前两节点本人占用0，启动后4≤6；/data1独立quota861.2GiB/1TiB，新增峰值预算20GiB、投影881.2GiB，共享83TiB。精确命令与资源记录`runs/analysis/video_functional_20260911/teacher_vl/launch_contract.json`，日志`train.log`；输出`runs/outputs/video_functional_20260911/teacher_vl/`。100/200全部物化与评测请求已准备，尚未启动评测，训练及评测共享Owner全局额度。
