...
def _policy_rng_seed_for_logical_batch(
    ...
    *,
    ...
    if (
        ...
        raise ExpertManifoldError("PVJFC action-query randomness changed")
...
def _batch_task_id(batch: Mapping[str, Any]) -> int:
    ...
    if not isinstance(values, torch.Tensor) or values.ndim != 1:
        raise ExpertManifoldError("PVJFC action batch lost task identity")
    unique = values.unique()
    if unique.numel() != 1:
        raise ExpertManifoldError("PVJFC action batch crossed tasks")
...
def _task_objective(
    ...
    *,
    ...
    if _batch_task_id(batch) != task_id:
        raise ExpertManifoldError("PVJFC sampler and action batch disagree")
    ...
    if primary_demo == companion_demo or companion_demo in excluded:
        raise ExpertManifoldError("PVJFC paired videos lost cross-episode separation")
    ...
    if kind == "wrong":
        ...
        if wrong_primary == wrong_companion:
            raise ExpertManifoldError("PVJFC wrong-video pair collapsed")
    ...
    if runtime.args.mode == "mechanism-profile" and task.ordinal in _PROFILE_TASK_ORDINALS:
        ...
        if len(fixed_query) != 4:
            raise ExpertManifoldError("PVJFC fixed-action profile query changed")
    del policy_batch
...
def _gather_paired_video_rows(
    ...
    if not 0 <= len(local) <= maximum_local:
        raise ExpertManifoldError("PVJFC local task coverage changed")
    payload = torch.zeros(
    ...
    if gathered_payload.shape[0] != _TRAIN_TASK_COUNT:
        raise ExpertManifoldError("PVJFC padded gather lost train24")
    ...
    if not torch.equal(
        ...
        raise ExpertManifoldError("PVJFC full96 task order changed")
...
def _gather_task_records(
    ...
    if len(result) != 24 or [int(row["task_ordinal"]) for row in result] != list(
        ...
        raise ExpertManifoldError("PVJFC macro did not cover train24")
    return result
...
def _collect_local_objectives(
    ...
    *,
    ...
    if len(jobs) != _TRAIN_TASK_COUNT:
        raise ExpertManifoldError("PVJFC lost train24 jobs")
    queue_path = (
        Path("/tmp")
        / f"ember-pvjfc-{os.getuid()}"
    ...
    if any(parameter.grad is not None for parameter in runtime.policy.parameters()) or any(
        ...
        raise ExpertManifoldError("PVJFC touched frozen parameter gradients")
...
def _macro_record(
    *,
    ...
    if counterfactual_counts != {"reversed": 8, "shuffled": 8, "wrong": 8}:
        raise ExpertManifoldError("PVJFC negative schedule changed")
    ...
    if not all(
        ...
        raise ExpertManifoldError("PVJFC metric became non-finite")
    return row
...
def _run_one_macro(runtime: V6PriorRuntime, *, macro: int) -> dict[str, Any]:
    ...
    if profile and _base_versions(runtime) != versions_before:
        raise ExpertManifoldError("historical v6 state changed during PVJFC profile")
    row = _macro_record(
        macro=macro,
        schedule_macro=schedule_macro,
        records=records,
...
def finalize_args(args: argparse.Namespace) -> argparse.Namespace:
    for name in ("config", "source_run", "checkpoint", "tokenizer_path", "data_root"):
        ...
        if not path.exists():
            raise ExpertManifoldError(f"missing PVJFC Writer path: {path}")
    ...
    if args.resume is None:
        if args.output_dir.exists() and (
            ...
            raise ExpertManifoldError("fresh PVJFC Writer output is not empty")
    elif (
        ...
        raise ExpertManifoldError("PVJFC Writer resume output ownership changed")
    if args.config != V6_PRIOR_CANONICAL_CONFIG.resolve():
        raise ExpertManifoldError("PVJFC Writer requires the canonical config")
    load_v6_prior_config(args.config)
    if args.num_workers < 0 or (
        args.stop_after_macro is not None and args.stop_after_macro <= 0
    ):
        raise ExpertManifoldError("invalid PVJFC Writer worker or stop boundary")
    return args
