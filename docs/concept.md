...
语言/单帧/无序参照用于辨识知识来源和学习机制，其差额不替代这个目标，也不要求先完成所有对照才能研究利用方法。
MT-BC在此是最终性能参照，不是待诊断的技术故障；某次未完成任务不能被命名为EMBER应去解决的“MT-BC失败问题”。
方法推导首先解释EMBER如何从教学获得能力，再由匹配比较判断其优势，不以先诊断MT-BC为前提。
