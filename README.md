...
**夹爪执行无关的幅度信用会实质改变完整Writer方向，但去除后两组初态的梯度仍不一致。** 下一项仅用96条配对闭环检验这项修正是否有实际收益；两个固定单步候选、无新梯度采集，不启动长程RL。尚无经过验证的统一根因或修复。
...
| [Findings](findings.md) | 编号的跨轮发现；近期冻结诊断见§135–139，关系支持及后继映射/学习诊断见§140–148 |
