...
5. **当前交付：固定A900的视频特异性。** 复用correct400，依次完成same-task-other、cross-suite-wrong、
   no-video，再做shuffled／reversed各400。复用固定初态、env／policy RNG和视频ordinal，逐task每臂全50视频各一次；
   no-video按既有零LoRA合同单独说明。汇总逐task／suite、breadth、保持／新增／丢失和任务bootstrap区间。
   仅作冻结模型的封闭描述，不选checkpoint或反馈训练／架构；交付结果后停下讨论，Test及其余训练仍暂停。
