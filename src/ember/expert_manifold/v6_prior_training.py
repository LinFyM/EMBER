...
def _gather_task_records(
    ...
    if (
        len(result) != 24
        or [int(row["task_ordinal"]) for row in result] != list(range(24))
    ):
...
def _run_one_macro(
    ...
    *,
    ...
    runtime_metrics = _runtime_maximums(
        runtime.context, step_started, input_wait
    )
...
def finalize_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.resume is None:
        ...
        raise ExpertManifoldError(
            "residual Writer resume output ownership changed"
        )
