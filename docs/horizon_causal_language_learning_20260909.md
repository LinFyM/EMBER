...
## 最终结论与证据边界（2026-09-10交付）

**本轮授权内的学习对照、固定复核、行为定位和轻量机制分析全部完成；已停在正式方法修改/采纳与下一轮训练之前，等待Owner复核。** 没有未完成面板或本任务GPU进程，没有继续401步或另起完整训练，未使用Test、held梯度或最终视频controls。后文保留结果前注册与阶段记录，其中旧“下一步/等待”均不恢复执行。本报告不声称已唯一解释旧新全部分差，也不把达到授权边界写成科学问题全部解决。

最明确的修正证据是：**当前Compiler额外仿射query支路对本协议的未见任务表现有局部负贡献；关闭它的净收益方向跨两次初始化和另一套正确视频关联保留。** checkpoint分析进一步定位到语言共用位移在LayerNorm前压缩target/rank检索差异，及静态语言读出/第一层Q的较大训练漂移；尚未把查询尺度、真实attention、共享reader梯度与后续补偿的作用单独分开。删除支路不是删除exact language或视频Value，也不是完整稳定修复。

**当前同target跨rank A/B两侧D绑定不宜作为修复方案采纳。** 它的早期验证收益115对103到400反转成82对90；训练任务却由32升至60、超过all49且保留29/32个早期成功。共享并未解决训练获取与未见任务保持的分歧，BBQ和Long退步仍在。这个否决只限实际参数绑定、all前端/Compiler背景、seed7和固定400预算，不否定所有共享，更不归因于纯减参。

| 实际学习臂 | validation200→400 | train96 200→400 | validation自身相邻 保留/新增/丢失 | 对当前问题的结论 |
|---|---|---|---|---|
| all / init7 | 103→90 | 41→49 | 56/34/47 | 训练获取与未见任务保持分离 |
| 仅删除Compiler额外query / init7 | 108→126 | 40→56 | 82/44/26 | 净学习与局部保持改善，仍有明显丢失 |
| all / init11 | 100→108 | 41→56 | 62/46/38 | 净增不能掩盖BBQ崩落 |
| 仅删除Compiler额外query / init11 | 104→119 | 44→61 | 67/52/37 | 验证净收益方向复现，稳定保持修复未复现 |
| 仅绑定同target跨rank D / init7 | 115→82 | 32→60 | 59/23/56 | 早期验证优势不保持，后期训练改善不能替代迁移 |

Compiler两个候选均在已观察的200→400区间获得净能力；**没有证据宣布它们已平台、已穷尽或再训无效**。现有两点也不能保证继续训练能达到严格>145并稳定。停止新增完整训练由Owner授权上限决定。共享的两节点退步同样不是对更远未来的数学证明，但足以否决“该绑定已修复本轮迁移/保持”的具体主张。

完整数据链是：exact language与有序真实视频→冻结vision/Gemma prefix→Action Meta读取完整50H→四组逐帧local/视觉对应与过去时间组织→P4视频Value→Compiler的target/rank代码→native D生成唯一38-target LoRA→冻结source在自己的当前观测/state上闭环执行。图接通不是机制兑现；下面按这条链区分证据等级。

| 接口或机制 | 已有的区分证据 | 证据能定位到哪里 | 尚不能推出什么 |
|---|---|---|---|
| 额外Compiler查询组织 | 固定local的2×2 fresh学习中，删除Compiler在H-read开/关背景的200效应+5/+16、400效应+36/+20；init11末期+11，init7换视频末期+44 | 支路对学习所得策略有局部因果贡献；H-read没有相同一致方向 | 不能单独定责纯文本内容：Compiler删除含bias，H-read置零输入仍留bias |
| 共用语言位移与LN/Q几何 | 八checkpoint实际选参计算：all第一层608个Q的差异幅度仅同权重置零反事实的3.5%–4.6%，后期Q/语言读出漂移较大 | 相加→LayerNorm→第一Q投影确有检索差异压缩；reader由静态与上下文支路共用 | 没有真实K/V attention或中介对照，不能说已证明饱和、选错帧或唯一根因 |
| D共享的函数空间和优化 | 只改同target跨rank D绑定；200/400 train与val排序发生反转；源码证实rank梯度先求和再进入AdamW | 绑定同时改变方向字典、跨槽梯度耦合及优化器状态；不是纯参数量实验 | 不能将效果归给所有共享、单独的正则化或跨task泛化保证 |
| 实际因子使用 | 共享200/400的B/BA近单方向，而D_B自身有多方向；全部32固定条件/38target一致 | 定位到代码经U_B/GELU与字典的实际使用，形式rank16未被硬降 | 低有效秩与train60并存，不证明它是缺陷、必须加rank损失，或不同task输出相同 |
| 保持与行为目标 | all所选BBQ回放转向绿色干扰瓶；Compiler候选对应回放更多围绕正确瓶；共享完整验证仍大量丢失BBQ/Long | 某些退步实例首先表现为对象选择；共享末期没有修复未见任务保持 | 共享没有新增轨迹，不能把all的绿色瓶行为当成其已观察事实；Long部分实例则是选对后执行失败 |
| 监督与读取边界 | FM/held动作误差不能一致排序闭环；source/Gemma prefix冻结而Action Meta可学习；train24约束有限 | 当前监督优化示范状态平均速度场，没有直接约束闭环旧成功集合或恢复状态 | 尚未单独识别prefix可学习性、task数量、状态覆盖或旧新配方效应；不能直接宣布必须RL或换loss |

**建议的优先级：** 保留“移除额外Compiler query”为最有实测依据的局部修正候选；围绕已定位的身份query/语言位移尺度及共享reader共同适应讨论下一设计。若未来仍保留额外语言检索，分别归一化身份与语言、显式控制组合尺度是机制明确但未经验证的选择。当前不采纳D绑定，不因近rank1就加正交/rank损失，不恢复旧架构枚举或seed/LR小扫。扩大meta-task、学习prefix、补闭环状态覆盖均仍需各自的识别证据，不自动进入实施。

**历史分差保留边界。** v5.2 old900的132含Spatial19，当前Compiler init7的126仍仅Spatial1；总分接近未恢复旧关系任务能力。v6 old500为121、task-complete400为143，同类纯FM和4-task更新曾更强，反对把FM/四任务组织普遍判为无效。旧新Meta范围、架构、teacher/query池、同episode排除、LR/分组并未严格配平，不能将本轮+36换算成解释全部历史差距的比例。Goal23与Long32在旧曲线也长期弱，不能全算为Horizon新丢失。细节见[历史与协议审计](horizon_k1_evidence_review_20260909.md)、[冻结与局部诊断](horizon_k1_causal_diagnostics_20260909.md)。

原件索引：语言与检索四个新学习臂3968行、Compiler固定确认12面板3584行，以及最后共享4面板992行各自保留完整raw rows、逐task/suite与实际strict配对；后文均指向对应索引。当前最后一轮总索引为`runs/analysis/horizon_relation_writer_20260908/causal_learning_20260909/sharing/completed_matrix.json`，机制原件在同原因分析root的`mechanism/`。所有数字属于实际单checkpoint，未融合、取union或用于正式选点。
...
## 输出D绑定最终完整裁决：训练获取更强，未见任务保持未修复

最后validation400完整82/400，对同节点all90为R/G/L63/19/27、churn46/J=.5780、breadth6→5。自身115→82为59/23/56、churn79/J=.4275、breadth7→5，仅保留51.3%的早期成功；all自身保留56/103=54.4%。较小churn或略高J均不能掩盖净能力与breadth下降。400 S/O/G/L为1/42/35/4，对all2/44/35/9。

共享BBQ26→3仅保留3、丢23；两条Long的早期10+3个成功全部丢失，末期4个均为新成功。其它Object35→39仍有27保留/12新增/8丢失，不能概括为所有任务同时停止学习。下表global ID遵守固定split，R/G/L均为该共享run自身200→400。

| global task | all200→400 | 共享200→400 | 共享 保留/新增/丢失 |
|---|---|---|---|
| 1 | 1→1 | 2→0 | 0/0/2 |
| 3 | 0→1 | 1→1 | 0/1/1 |
| 11 | 29→43 | 35→39 | 27/12/8 |
| 13 | 25→1 | 26→3 | 3/0/23 |
| 23 | 1→0 | 0→0 | 0/0/0 |
| 26 | 36→35 | 38→35 | 29/6/9 |
| 31 | 6→9 | 10→4 | 0/4/10 |
| 32 | 5→0 | 3→0 | 0/0/3 |

训练task的相反结果同样必须保留：共享32→60，对all400多11，旧成功29/32保留；validation却115→82、旧成功59/115保留。这是本轮最清楚的“训练行为改善不等于未见任务迁移/保持改善”反证。低有效秩在两个共享节点均存在，200验证还高于all，400却低于all；它不能单独解释这种时间上的效应反转。没有识别到一个能由提高rank、更多参数或泛称共享就自动解决的根因。

完整train24的每task四状态结果如下，R/G/L同样为共享自身相邻；aggregate之外的弱task不能省略。
...
最后六个validation worker及wrapper全部exit0、actual同节点/相邻配对通过，墙钟1724.22秒；全轮共享四面板992行均完整。两节点完整checkpoint、累计1600条件/102400queries和held3072信息墙核验通过，所有训练/物化/eval/本地观察进程均结束。精确原件`sharing/step400/{validation,train96}_analysis/`及`sharing/completed_matrix.json`；科学运行面c63f55dc、后续评测e7447291仅含资源准入修正，探索科学实现未合入main。

**裁决只否定当前A/B两侧、同target跨rank D绑定在all背景下已修复迁移/保持的主张。** 它不支持正式采纳，也不否定其它共享方式、其它前端背景或更远学习节点。历史Target-Owned的负例与本轮同向边界仍保留；不能为了挽救此结果叠加Compiler删除、改seed/LR/rank或追加完整训练。后续仅由Owner复核后另行授权。
...
为区分字典本身与实际使用，进一步只读共享200的38个D_B（约40MiB）。D_B第一奇异方向能量均值65.02%，逐target38.54%–74.46%，stable rank均值1.5723；前4方向89.50%。1.53秒完成，原件`mechanism/dictionary_geometry_step200.json`。D_B自身并没有达到生成B的近乎单方向集中，**因此仅说“共享字典只能表达rank1”不符合实际参数**。 全部目标中还包含输出native维度为32或256的映射，它们并不受256维字典的更低维上限限制，实际B仍同样集中；因此仅用“native子空间被截成256维”也不能解释这一普遍现象。更窄的定位是：代码经过U_B/GELU与D_B映射后，对字典的实际使用高度集中。尚不能由此分开C本身差异小、U_B/GELU压缩差异与代码选择D_B主方向的共同适应；也没有直接证明仅恢复B多方向就会修复闭环。
...
400的496条件bank后来完整物化并公开核验通过，774.79秒/exit0，全部task/state/video选择与基线一致。按同一预登记选择读取的400输出几何也已完成（4.99秒）：共享B第一方向能量train/val为99.99894%/99.99895%，BA为99.99960%/99.99960%，全部1216个B仍>99.9924%、BA>99.9759%。相应all的B为51.52%/51.51%、BA为71.46%/71.47%，比200更多方向参与。共享D_B自身的第一方向均值则从65.02%降至57.86%（范围39.67%–71.88%、stable rank均值1.7606），即字典更分散，实际B的使用仍近单方向。原件`mechanism/{output_geometry,dictionary_geometry}_step400.json`。

这排除了“只在200这个节点刚好近单方向、到400已恢复多方向输出”的具体解释；不证明未来永远不变或行为已经平台。独立all的更多方向伴随其验证103→90，也再次表明不能用rank几何单独排序策略能力。共享400行为面板尚在原定评测，最终权衡必须接着读其完整闭环与成功得失。

### 共享400训练任务完成：早期获取代价反转，旧成功保持较强

400 held-video train96完整60/96，对同节点all49为R/G/L41/19/8、churn27/J=.6029，breadth均20；S/O/G/L18/19/14/9，对all12/16/15/6。两者的差异主要是Spatial+6、Object+3、Long+3、Goal−1，不能概括为每suite全部改善。三worker及wrapper全部exit0、actual同节点与自身相邻配对通过，墙钟955.20秒；完整逐task/suite及原行在`sharing/step400/train96_analysis/`。

共享自身200→400为32→60、R/G/L29/31/3，保留90.6%的早期成功，breadth16→20；Spatial旧7个与Long旧4个全部保留，丢失来自Goal2个、Object1个。其churn34/J=.4603主要由31个新增带来，不能把34读成遗忘数量，也不能单用比all的J=.5更低来否定较高的旧成功保持。all自身41→49为30/19/11，保留73.2%。这里是训练task跨独立teacher/初始state面板的保持，未见task结论仍等待最后validation400。

因此，**200的训练获取劣势到400已经反转；近单方向输出并没有阻止本面板继续获得能力与保持旧成功。** 这反对把实际低有效秩直接命名为持续拟合缺陷，也反对“只有恢复更高rank才可能有效”的建议。输出使用集中是可靠结构诊断，但它在此学习过程中的行为作用仍可能兼含有益约束、优化轨迹变化和未覆盖能力代价。held FM400仍比all更高，而闭环更好，进一步要求先按真实行为选择方法。
...
对共享的建议已由本轮完整结果收窄：不将当前绑定采纳为迁移/保持修复。若有必要在未来分辨A/B两侧约束，单独绑定B、保留独立A可检验共同输出方向复用与输入残差限制的不同作用；它仍改变B侧AdamW，不能称纯减参对照。当前没有该方案的效力证据或新增训练授权。类似地，扩大meta-task、学习prefix或补闭环状态覆盖分别针对泛化约束、可学习证据与执行信用；它们尚未被本轮独立识别，不能当作接下来必须机械执行的清单。
