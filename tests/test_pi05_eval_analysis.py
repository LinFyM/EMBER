...
def test_checkpoint_curve_routes_mgci_as_fresh_0_5_10_family() -> None:
    results = {
        f"mgci-{macro}": _result(macro, "correct", set(), family="mgci")
        for macro in (0, 5, 10)
    }
    analysis = checkpoint_curve_analysis(results)
    assert analysis["method_family"] == "v6_mgci_jc_v1"
    assert set(analysis["panels"]["correct400"]) == {"0", "5", "10"}
