...
2. **进行中：失效定位。** 沿视频证据→过程表示→LoRA→独立闭环建立竞争解释。先补已有C100/C200缺失的正确/错误视频机制状态，与匹配S和source比较、复用off400九臂；无新学习。结果前合同已登记于`runs/analysis/video_mechanism_20260911/registration.json`，1344新rows；source复用96rows。不把梯度、输入完整性、attention或一次局部拟合当成理解或根因。
