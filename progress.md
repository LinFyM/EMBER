...
## 当前工作（2026-09-17，统一架构设计已完成）

Owner要求重新设置goal，进一步继承v5.2处理原则而非机械保留模块，比较更协调统一的架构，说明此前多通路推导
与上一轮方案的取舍，并主动检查可预见问题，形成清楚、稳定的设计立场。本轮只分析与设计文档；不启动实现、训练或评测。
完整证据审计继续复用；两项只读并行工作分别追溯多通路原始论证和复核数学性质，主代理整合并作出最终选择。
选择[统一Writer设计](docs/v52_evidence_based_writer_design.md)：native1–9→联合Z/H块×2→双残差写回→native10–18及final norms→
同构块×2→唯一memory。单一参数槽状态先从已联合的语义位置取得内容，再读取全memory；保留归一化、三Meta和八组完整A/B heads。
不再独立建立Core/P或专用AdaLN。此前丰富内容、跨帧关系、原生消费和原上下文保留的职责均明确对应，
首轮保留旧尾端出于控制改变量，不是已证明旧模块必需。首轮版本保存在Git `426dc5be`。
真实接口、静态重复不变性、地址/Value边界、有限信用开启、完整H与有损投影的区别及参数/复杂度已复核；
解析计数13,451,008参数，尚无runtime测量。纯FM的静态捷径、实际native消费、迁移/保持与合理条件鲁棒性均未冒充结构保证。
14项可预见问题的固定立场、whole-method匹配比较、内部消费增量的独立比较要求及行为裁决边界已写入设计。
最终文档差异、新增本地引用与授权状态核对通过，由本次提交统一交付。没有源码改动、模型forward、GPU、held动作或Test使用；所有旧实验继续暂停。
这是一份分析交付，**不注册为可执行active design**，也不恢复此前实验goal或run；只有一个canonical设计入口。

## 上一轮已完成（2026-09-17，证据审计与首版架构设计）

Owner已明确要求设立新的分析/设计goal并允许充分时间：全面审计现有正负证据和验证强度，从实际task样例及理论数学
逐步形成以v5.2为基座、能同构加深、结构清晰优雅且匹配实际联合纯FM训练的完整架构。主要问题是视频特异性丢失与
脆弱性，并须保留已获闭环能力、解释保持性边界。不得用成功和失败方法共有的流水线冒充v5.2更有效的原因。
三项只读历史审计已完成，主代理已整合实际train0/20/37/39的BDDL/RGB、数学推导和完整设计。
交付为[证据审计](docs/v52_evidence_audit_20260917.md)与首版架构推导（Git `426dc5be:docs/v52_evidence_based_writer_design.md`），全部步骤已逐项复核。
审计保留46个编号证据组及12组bank中间路线，纠正PNBTT E2未跑、polar只有profile、width256没有闭环及Layered旧重复映射等边界；
旧v5.2原132仍有无放回合同，不能因日期早而一概降为重复视频结果。
选定的交付架构在native第9层后联合处理真实task-span Z与完整H50，同构block内做帧内混合、双向时间attention和FFN，
N=4后同时残差写回Z/H，再经原生第10–18层；保留v5.2 Core/P/归一化Compiler/8family heads。
该设计让时间信息能够进入Core，且重复静态视频不靠位置产生变化；这些是结构性质，不是视频有益性或保持性的实验结论。
Fresh联合纯FM、初始2400更新建议窗口、matched dual/H50基线与全部信息墙写在设计中；成本和行为尚未验证。
本次没有改运行源码或启动训练/评测；新文档是分析交付，**不注册为可执行active design，不恢复旧goal或run**。
A3000及SFT均保持已结束状态。新增文档本地原件链接、关键曲线/预算、参数和复杂度算式、diff及授权边界已核对；
其中46池同episode排除的有限依赖、P并非数学上只能小修正等前提已补正。
本次完成的是分析/设计goal；新架构的学习、性能、资源峰值与鲁棒性均尚未实测，旧实验目标不随之恢复或标完成。
