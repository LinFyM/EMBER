...
3. **当前：检验输出独立性。** 选择已CPU准备的同target跨rank D绑定，以原all seed7为共同基线，只改这个参数绑定；不同时叠加Compiler删除，不恢复旧v6/VL。沿用合法池、4×64、LR3e-5/warmup8和固定200/400行为/held3072。历史Target-Owned负结果仍有效，本次只识别当前前端与协议下的单接口效应。先真实profile和当时GPU/独立quota/精确launch，随后隔离fresh学习；主方法不改、不额外扫seed/LR/rank。具体候选与结果前合同见报告末节及隔离候选文档。
