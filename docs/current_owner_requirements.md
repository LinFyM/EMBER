...
## 当前阶段的优先级：统一架构与设计立场复核（Owner 2026-09-17）

Owner再次明确设立分析/设计goal：不能只在v5.2上叠加模块，应提炼其有效的处理原则，认真比较更协调统一的架构；
必须解释此前逐步推导的多通路方案如何被保留、合并或放弃。交付须有完整、主动检查过反例的工作原理与明确取舍，
不能等owner逐个提出问题后才补认已可预见的缺口。设计信心来自一致的证据和推导，不承诺未经实验的性能，
也不以“没有matched删除实验”自动禁止实质重构。只分析与修订设计文档，不实现或启动实验。
上一轮证据审计继续有效。本轮选定[统一Writer设计](v52_evidence_based_writer_design.md)：原生中层/末端同构联合Z/H处理，
一次中层双写回，再以内容起始的单一参数状态生成完整LoRA；旧Core/P/AdaLN的职责由统一表示与连续读取承接。
首轮保留全部旧尾端的方案通过Git保存，不再是并列canonical候选；新设计尚未实现，不构成实验恢复授权。

## 上一轮已完成：全面证据推导与架构设计（Owner 2026-09-17）

Owner明确设立无时间上限的分析与设计goal：认真、全面整理现有成功/失败、充分/不足验证的证据，
从真实task样例和已有材料出发，用理论与数学逐步推导一个以v5.2为基座的完整架构。允许实质重构；
须解释v5.2相对失败方法更有效的具体原因，并据此设计，不能以两者共有的video/native→Writer→完整A/B→FM框架充当原因。
结构须清晰合理优雅、避免无谓复杂度，并能通过复制同构层加深、增加参数量；须与实际训练方法一致。
优先尝试解决视频特异性丢失及其对合理条件变化的脆弱性，同时保留已有闭环能力；训练保持性及未验证假设须如实处理。
Owner已表示130多的实际性能有保留价值，当前主要矛盾是视频教学作用是否可信；长期>145资格仍保留。
本次只授权分析、证据整理和设计交付，不实现、不启动训练/评测；所有步骤认真完成前不得标记goal完成。
下述A→分析→一轮B是历史授权与事实，实验部分继续暂停，不因本次设计goal恢复。

## 历史阶段：弄清A，分析改进，完成一轮B（Owner 2026-09-16）

Owner明确调整goal，自主完成三步：继续对齐A的训练和正式评测，弄清后续性能是否像原始v5.2一样不稳定，
以及视频特异性会改善还是恶化；随后结合原始v5.2至今近两个月的实验，仔细分析问题、原因和改进依据；
最后把改进称为本轮B，完成一轮正式训练、评测和同样的分析，无论解决与否都直接向owner汇报，不另写报告。
期间正常推进共享SFT的训练和评测。此前A900后暂停、SFT只到400及不运行新B的边界由本次授权替代。
新的B待A全貌及历史证据分析后形成，不自动沿用此前双视角／learned H-read B，也不恢复旧C。
>145仍是项目性能资格，不能把本次一轮B扩大成无限搜索或以获得通过结果作为结束前提。
具体节点、资源及解释边界见[source时间对齐与v5.2复核](source_alignment_v52_plan.md)和progress。

2026-09-17追加：先看A1800的完整正式结果，若有反弹就继续往后训练；owner希望看到明显过拟合趋势再停止A。
1800不再是自动终点。结合训练任务与未见任务表现、相邻成功集和多节点趋势判断，不能把单次回落或两侧同时下降
直接命名为过拟合；新的节点沿原配方与时间轴登记续训，不据视频controls决定是否延长，也不改变后续分析与一轮B的目标。

Owner纠正：旧v5.2的强视频特异性是“没有在新A900复现出来”，不能淡化为“没有稳定建立”。
底座／采样变化限制根因归属，但必要纠错或合理训练条件调整不能替方法表现开脱。
本轮允许预注册节点的视频特异性演化诊断，并将A的完整事实用于问题分析与改进依据；不按controls选点，
不训练打乱／倒序判别或把这些对照加入loss，Test保持关闭。历史封闭Test及旧诊断的既定边界不追溯改变。
