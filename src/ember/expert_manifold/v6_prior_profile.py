...
def _lora_response_row(
    ...
    if set(before) != set(after):
        raise ExpertManifoldError("PVJFC profile LoRA topology changed")
    ...
    for name, before_a in before.items():
        ...
        if b_name not in before:
            raise ExpertManifoldError("PVJFC profile lost a LoRA factor pair")
        after_a = after[name]
    ...
    if any(value <= 0 for value in counts.values()):
        raise ExpertManifoldError("PVJFC profile LoRA response is empty")
    totals = torch.stack(
...
def _fixed_action(
    ...
    *,
    ...
    if action.ndim != 3 or action.shape[0] != 1:
        raise ExpertManifoldError("PVJFC fixed-action profile output changed")
...
def profile_lora_response(
    ...
    if correct_motion.shape[0] != 2 * _TASK_COUNT:
        raise ExpertManifoldError("PVJFC profile response topology changed")
    ...
    try:
        for objective in local:
            ...
            if objective.fixed_policy_query is None:
                raise ExpertManifoldError("PVJFC profile lost fixed-action query")
            for view_index, (view_name, view) in enumerate(
                ...
                if graph is None or not graph.correct_lora:
                    raise ExpertManifoldError("PVJFC profile lost before graph")
    ...
    if len(rows) != 8 or observed != expected:
        raise ExpertManifoldError("PVJFC profile response lost four-suite paired views")
    return {
        "probe_rows": len(rows),
...
def profile_task_local_motion(
    ...
    if cotangents.shape[0] != 48 or full_motion.shape[0] != 96:
        raise ExpertManifoldError("PVJFC task-local profile topology changed")
...
def profile_passes(
    ...
    for name in ("update", "application", "task_local_motion", "lora_response"):
        if not isinstance(row.get(name), Mapping):
            raise ExpertManifoldError("PVJFC mechanism profile evidence is incomplete")
    update = row["update"]
