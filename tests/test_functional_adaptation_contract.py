...
def test_process_contrasts_never_reintroduce_target_overlap_tasks() -> None:
    protocol = load_meta_protocol(PROTOCOL)
    active = set(protocol["active_source_task_ids"])
    grouped = {
        int(task_id)
    ...
    assert "does not prove equal-endpoint" in protocol["known_process_data_gap"]
