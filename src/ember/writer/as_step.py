...
def _cumulative_counts(
    ...
    unique = sum(
        runtime.sampler.batch_size_for_step(step) for step in range(completed)
    )
    ...
    return (
        unique * scale,
        completed * runtime.batch_size * scale,
        conditions * scale,
    )
...
def _step_metrics(
    ...
    *,
    ...
    values: list[torch.Tensor],
    details: list[Mapping[str, Any]],
    matching_probability: torch.Tensor | None,
    grad_norm: torch.Tensor,
    applied_lr: float,
    video_metrics: Mapping[str, int | None],
) -> dict[str, Any]:
    completed = step + 1
    ...
    wrong = (
        reduce_mean(float(values[1]), runtime.context)
        if len(values) == 2
        else None
    )
    probability = (
    ...
    return {
        ...
        "global_unique_action_queries_this_step": (
            observed_batch * runtime.context.world_size
        ),
        "global_policy_samples_this_step": (
            runtime.batch_size * runtime.context.world_size
        ),
        ...
        **video_metrics,
        ...
        "global_policy_samples_per_second": (
            runtime.context.world_size * runtime.batch_size / step_seconds
        ),
        "global_unique_action_queries_per_second": (
            runtime.context.world_size * observed_batch / step_seconds
        ),
