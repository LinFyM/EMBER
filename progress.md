...
更新时间：2026-09-10 CST。**Owner最新请求加入双视角：输入实现与验证已完成，未启动训练。** 原因分析已交付；本次授权仅推进同步双相机输入能力，不自动采用Compiler删除/D共享候选，也不恢复旧run。

**授权边界：** 此前最后共享完整训练已结束，新增训练/延续仍待Owner复核。最新双视角请求授权现有数据、observer、缓存、物化接口及相关验证的修改；不会产生模型更新、Test或最终视频controls。现有400步单视角checkpoint仍按原视角合同解释；若后续批准延续，按Owner最新要求每100步保存并做strict400。

## 双视角输入实现

设置`observer.camera_view=dual`后，同一teacher episode的agentview与eye-in-hand按相同真实帧索引进入原生双相机prefix；K仍计episode数量，源图像、Z/KV、完整50H及单套76张量输出职责保持。视角配置进入已有run/bank合同，拒绝相机缺失/不同步、数据与observer不匹配，以及跨视角exact-resume或bank/resident复用。缺省仍是历史单视角；本次不静默切换现有训练配置。

四个相关测试文件86项已通过。train task0/demo0完整98帧按stride5含末帧取21时刻，实际双视角smoke完整exit0：K1，prefix538tokens，证据512图像+18任务tokens，R为21×50×1024，完整76个A/B有限值，144个Meta梯度张量有限且非零，source无梯度。GPU01单卡进程内12.21秒、峰值10.51GiB，无optimizer或参数更新，进程已结束。使用既有单视角权重只验证输入链，不是双视角训练结果或性能对照；原件`.codex/tmp/dual-view-input/`。
