    ...
    *,
    ...
    with torch.no_grad(), authorities.observer.action_meta.installed(expert):
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            prior_compilation = authorities.model.compiler(program.prior_only())
    with torch.no_grad(), torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        prior_surface_loss = exact_effective_update_loss(
            prior_candidate, authorities.prior_state, authorities.contract
        )
    stored = {
    ...
    prior_stored = {
        ...
        for name, value in prior_candidate.items()
    }
    validate_lora_state(prior_stored, authorities.contract)
    prior_path = output_dir / (
        f"task_{task.ordinal:02d}_global_{task.global_task_id:02d}_prior.safetensors"
    )
    save_file(prior_stored, str(prior_path))
    diagnostics = {
    ...
    row = {
        ...
        "prior_projected_adapter": str(prior_path.resolve()),
        "prior_projected_adapter_bytes": prior_path.stat().st_size,
        "video_demo_indices": list(packed.demo_indices),
        "visible_video_count": len(packed.demo_indices),
        "program_route": "frozen_q_pi_single_surface_compiler",
        ...
        "prior_surface_effective_update_loss": float(prior_surface_loss.detach()),
        "exact_owner_attention": float(compilation.exact_owner_attention.detach()),
        "prior_exact_owner_attention": float(
            prior_compilation.exact_owner_attention.detach()
        ),
