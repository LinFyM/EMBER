"""Semantic Core, per-video Procedure, and slot-normalized LoRA compiler."""
...
def _apply_rope(value: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
    ...
    angles = positions.to(torch.float32)[:, None, :, None] * frequency[None, None, None]
    cosine, sine = torch.cos(angles).to(value.dtype), torch.sin(angles).to(value.dtype)
...
class TaskSelectedSemanticSetFusion(torch.nn.Module):
    """Read a permutation-invariant union of phase-aligned video evidence."""
    ...
    def forward(
        ...
        query = _split_heads(self.query(self.query_norm(text_queries)), self.heads)
        ...
        logits = torch.einsum("bhld,bhtld->bhtl", query, key) / math.sqrt(self.head_width)
        ...
        weights = weights.masked_fill(~valid_task_tokens[:, None, None, :], 0.0)
...
class RoPEContentBlock(torch.nn.Module):
    ...
    def forward(
        self, content: torch.Tensor, positions: torch.Tensor, valid_mask: torch.Tensor
    ) -> torch.Tensor:
        if (
            content.ndim != 3
        ...
        query = _apply_rope(_split_heads(self.query(normalized), self.heads), positions)
        key = _apply_rope(_split_heads(self.key(normalized), self.heads), positions)
        value = _split_heads(self.value(content), self.heads)
        allowed = valid_mask[:, None, None, :]
        if self.causal:
            causal = torch.ones(
                content.shape[1], content.shape[1], dtype=torch.bool, device=content.device
            ).tril()
...
class LanguageSemanticCore(torch.nn.Module):
    def __init__(self, *, width: int, heads: int, blocks: int) -> None:
        ...
        self.semantic_set_fusion = TaskSelectedSemanticSetFusion(width=width, heads=heads)
...
class TaskGroundedVisualTransitionFusion(torch.nn.Module):
    """Let each Action-Expert probe read within-video visual change."""
    ...
    def forward(
        ...
        if (
            ...
            or valid_task_tokens.shape != (action_probe.shape[0], grounded_evidence.shape[2])
        ...
        transition = torch.cat(
            (torch.zeros_like(grounded_evidence[:, :1]), grounded_evidence[:, 1:] - grounded_evidence[:, :-1]),
            dim=1,
        )
        ...
        query = self.query(self.probe_norm(action_probe)).reshape(
            batch * frames, 1, self.heads, self.head_width
        ).transpose(1, 2)
        key = self.key(self.transition_norm(transition)).reshape(
            batch * frames, task_tokens, self.heads, self.head_width
        ).transpose(1, 2)
        value = transition.reshape(
            batch * frames, task_tokens, self.heads, self.head_width
        ).transpose(1, 2)
        allowed = valid_task_tokens[:, None].expand(-1, frames, -1).reshape(
            batch * frames, task_tokens
        )
        attended = F.scaled_dot_product_attention(
            query, key, value, attn_mask=allowed[:, None, None], dropout_p=0.0
        )
        residual = self.output(attended.transpose(1, 2).reshape(batch, frames, width))
        return (action_probe + residual).masked_fill(~valid_frames[..., None], 0.0), transition
...
class CausalProcedureEncoder(torch.nn.Module):
    ...
    def forward(
        ...
        if not bool(valid_mask[:, 0].all()):
            raise VariableEpisodeInputError("Procedure must begin at phase zero")
...
class ContentCrossAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if self.rotary_keys:
            ...
            query = _apply_rope(
                query,
                torch.zeros(query_key.shape[:2], dtype=torch.long, device=query_key.device),
            )
        ...
        attended = F.scaled_dot_product_attention(
            query,
            key,
            _split_heads(self.value(memory_value), self.heads),
...
class PostFusionSlotBlock(torch.nn.Module):
    def __init__(self, *, width: int, heads: int) -> None:
        ...
        self.self_attention = torch.nn.MultiheadAttention(
            width, heads, dropout=0.0, batch_first=True, bias=False
        )
        self.ffn_norm = RMSNorm(width)
        self.ffn = torch.nn.Sequential(
            torch.nn.Linear(width, 4 * width, bias=False),
    ...
    def forward(self, content: torch.Tensor, routing: torch.Tensor) -> torch.Tensor:
        addressed = self.self_norm(content) + routing
        attended, _ = self.self_attention(addressed, addressed, content, need_weights=False)
...
class SlotNormalizedCoreProcedureCompiler(torch.nn.Module):
    """The exact historical v6 shared compiler for one LoRA."""
    ...
    def __init__(self, *, width: int, heads: int, initialization_seed: int) -> None:
        ...
        self.core_memory_norm = RMSNorm(width)
        self.core_attention = ContentCrossAttention(width=width, heads=heads, rotary_keys=False)
        self.procedure_memory_norm = RMSNorm(width)
        self.procedure_attention = ContentCrossAttention(width=width, heads=heads, rotary_keys=True)
    ...
    def _routing(self) -> torch.Tensor:
        expert = (
            self.query_table[: self.EXPERT_LAYERS * self.RANK].reshape(
                self.EXPERT_LAYERS, self.RANK, -1
            )
        ...
        action_in = (
            self.query_table[self.EXPERT_LAYERS * self.RANK : -self.RANK]
    ...
    def fused_slots(
        ...
        core_slots = self.core_attention(
            routing, self.core_memory_norm(core), core, valid_core
        )
        normalized_core = self.core_norm(core_slots)
        mask = valid_procedure[..., None]
        mean = (procedure * mask).sum(dim=1, keepdim=True) / mask.sum(dim=1, keepdim=True).clamp_min(1)
        centered = (procedure - mean).masked_fill(~mask, 0.0)
        procedure_slots = self.procedure_attention(
            routing + normalized_core,
            self.procedure_memory_norm(procedure),
            centered,
            valid_procedure,
            positions,
        )
        gamma, beta = self.modulation(self.procedure_norm(procedure_slots)).chunk(2, dim=-1)
    ...
    def forward(
        ...
        return (
            content[:, :stop].reshape(core.shape[0], self.EXPERT_LAYERS, self.RANK, -1),
            content[:, stop : stop + self.RANK],
            content[:, -self.RANK :],
        )
