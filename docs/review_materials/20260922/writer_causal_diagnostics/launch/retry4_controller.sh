...
WORKER="$STUDY/launch/retry4_run_worker.sh"
...
START_EPOCH=$(date +%s); DEADLINE_EPOCH=$((START_EPOCH + 7200)); D3_MIN_REMAINING=3900
SIGNAL=ember-writer-causal-diagnostics-20260922-retry4-complete
printf '{"started_epoch":%s,"deadline_epoch":%s,"wallclock_limit_seconds":7200,"d3_min_remaining_seconds":3900,"reused_complete_d1_function_probe_parts":true}\n' "$START_EPOCH" "$DEADLINE_EPOCH" >"$STUDY/launch/retry4_controller_started.json"
PIDS=()
finish() { code="$1"; printf '%s\n' "$code" >"$STUDY/launch/retry4_controller.exit"; tmux wait-for -S "$SIGNAL" || true; exit "$code"; }
...
run_finalize() { status="$1"; PYTHONPATH="$RUNTIME/src" "$PYTHON" "$RUNTIME/scripts/run_writer_causal_diagnostics.py" --output "$STUDY" finalize --d3-status "$status" >"$STUDY/launch/retry4_finalize.stdout.log" 2>"$STUDY/launch/retry4_finalize.stderr.log"; code=$?; printf '%s\n' "$code" >"$STUDY/launch/retry4_finalize.exit"; return "$code"; }
