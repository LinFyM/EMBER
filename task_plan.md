...
2. **当前：复核有实测收益的固定候选。** 结果前合同已写入`docs/horizon_causal_language_learning_20260909.md`末节：all/local_h_read成对optimization seed11、data seed7、fresh400及固定200/400完整行为；冻结seed7两节点另用schedule seed20260910配对正确视频，复用原LoRA；补原四state的BBQ候选回放。先验证收益/保持的可复现边界，不挑seed或单峰，不以126宣布整体修复或正式采纳。init11两臂已完整fresh400，四个checkpoint、累计实际曝光及held3072通过检查；独立初始化200验证已完成：all100、LH104，R/G/L76/28/24；BBQ27→35而双物体Long10→5，不能外推保持。四组496条件bank全部完成且actual配对通过，200 train96已完整all41/LH44、R/G/L37/7/4；all400验证108、train96为56均完整，相邻验证R/G/L62/46/38、BBQ27→5，其余七task73→103。LH400验证也完整119，相对all净+11（R/G/L86/33/22），自身104→119却丢37、breadth7→6，BBQ35→19只保留15/丢20。总分收益方向复现，稳定保持修复未复现；仅余LH400 train96在跑。换视频四面板全部完成：all99→82、LH101→126；LH相邻R/G/L71/55/30，BBQ26→31保留17、丢9，但Spatial5→0、moka4→0，不能称整体稳定。候选回放已完成。资源和实际进程见progress。
...
4. **先完成分析交付：** 主要因果贡献、经过复核的具体方案与实测效果、历史反证和未识别边界；剩余主要解释若仍有明确可行的区分实验，应先完成，不用单个正结果代替此前要求的分析。
...
Owner随后明确允许针对当前问题提出创新性架构并进行探索验证；这一授权与反对历史架构枚举重训并不冲突。候选不限于共享绑定或现有模块消融，须从当前机制缺口推导、说明新旧实质差异、以实际学习与闭环辨别。完整架构包并非一律不可探索，但不能把重训整个旧架构当成原因分析的默认前置。
