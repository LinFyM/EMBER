...
from ember.writer.architecture import V10_WRITER_PARAMETER_COUNT
...
AS_WRITER_LAUNCH_SCHEMA = "ember_pi05_language_axial_as_writer_launch_v10"
...
def resolve_runtime(
    ...
    if args.mode == "formal" and config["formal_run"].get("status") != "sealed":
        raise WriterModelError(
            "formal AS-Writer config is not sealed from the live v10 profile"
        )
...
def writer_trainable_contract(
    ...
    if (
        not names
        or parameter_count != V10_WRITER_PARAMETER_COUNT
