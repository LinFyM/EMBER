...
CONFIG="$ROOT/configs/gate_zero_action_expert_capacity_closed_loop.toml"
...
FIT="$ROOT/configs/gate_zero_mature_action_expert_lr_recovery.toml"
...
spec = tomllib.load(open(sys.argv[1], "rb")); root = pathlib.Path(sys.argv[2])
...
print(root / spec["authority"]["signed_ratio_result_relative_path"])
print(spec["resources"]["minimum_free_memory_mib"])
PY
)
source_checkpoint=${paths[0]}; fit_root=${paths[1]}; proposal_a=${paths[2]}
signed_ratio=${paths[3]}; minimum_free_memory_mib=${paths[4]}
command=(
  "$PYTHON" -m torch.distributed.run --standalone --nproc-per-node=2
  -m ember.gate_zero_support.capacity_closed_loop
  --config "$CONFIG" --gate-zero-contract "$GATE_ZERO" --phase0-contract "$PHASE0"
  --source-competence-contract "$COMPETENCE" --fit-contract "$FIT"
  --source-base-checkpoint "$source_checkpoint" --fit-root "$fit_root"
  --proposal-a-result "$proposal_a" --signed-ratio-result "$signed_ratio"
  --output-dir "$output_dir" --latest-link "$latest_link" --physical-gpus "$gpus"
)
...
for path in "$source_checkpoint" "$fit_root" "$proposal_a" "$signed_ratio"; do
...
[[ ! -e "$output_dir" ]] || die "refusing to reuse diagnostic output"
mkdir -p "$output_dir"
...
export CUDA_VISIBLE_DEVICES="$gpus" CUDA_DEVICE_ORDER=PCI_BUS_ID MUJOCO_GL=egl
export PYOPENGL_PLATFORM=egl HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1
export HF_HUB_DISABLE_TELEMETRY=1 TOKENIZERS_PARALLELISM=false PYTHONHASHSEED=20260719
export OMP_NUM_THREADS=2 TRACKIO_DIR="$EMBER_OUTPUT_ROOT/trackio" TRACKIO_STORAGE_MODE=sqlite
