...
- [ ] 对AS做不改架构的低方差functional训练修正：四卡时每rank每个optimizer update顺序处理2个独立task/video conditions，每个condition使用64个独立action queries并拆成4个16-query policy microbatches；因此每次更新仍覆盖8个独立conditions和512个queries。该设置只用于提高条件覆盖与梯度稳定性，不把batch size当作方法门槛；已有checkpoint能合规续训时不因GPU数量变化机械重训。现有`as_step.py`仍是唯一训练owner，不新增runner；profile通过后再决定从现有best继续还是fresh对照。
- [ ] 充分探索rank128 Source-SFT。四卡batch128 profile已完成，稳定吞吐约`36.18 queries/s`、峰值reserved `67.98GB`；旧八卡`step400=122/400`仍是全局incumbent，但它对应400次更新、204,800 queries和3,200个独立task-condition visits，不能把raw step机械映射到四卡轨迹。
