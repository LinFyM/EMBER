...
局部纠正场完整关闭后仅做了历史源码／原件与学习机制的只读复核，没有新增训练、评测、模型实现或实验登记。
冻结source导数用于参数生成的想法仍未采纳、未实施；不由该想法或下文旧自主授权启动工作。等待Owner明确恢复。

## 最近完成：局部纠正场完整比较关闭，复核整体学习机制（2026-09-14）
...
Owner要求“你先停一下，我想听你介绍下目前的情况”，随后继续讨论方法。整体goal未完成并保持暂停；
...
Owner最新澄清：核心要求是理解视频时序及其操作前置关系、任务语言中的“先A后B”；
不强制视频读取只看过去，允许完整视频双向解释。与此同时，有序模型必须强于匹配且独立训练的全帧无序模型，
不能取消这一方法证据要求。已同步稳定要求、concept与task plan；未选择新架构、改动旧实验合同或恢复执行。
...
100有序／无序的方向缺口J与整体幅度可消除H为.599946/.037671、.598835/.036758；
200为.582501/.047122、.578330/.056906。四个H−J task-cluster95%区间均严格负，各模型24/24task均值J>H。
最优privileged整体倍率只消除当前实际参数误差的3.87%–6.32%，空间内D的91.04%–94.22%仍需改变方向；
按登记停止仅整体gain修复的主要依据。它不排除未检验的层间方向机制，不把参数比例换算成闭环收益。

当前单位行A坐标的最小B范数比，逐条件中位数约256–387倍于正交行坐标；完整层／条件及能量汇总均保留。
这说明低固定A下界可能要求大的相消系数；范数有坐标依赖，不能直接认定有限模型容量上限、数值故障或修复方法。
200已见组67条／24task的task等权J仍为有序.585914／无序.585044；该缺口不能仅归未见video，分组不作因果推断。

下一项先区分可用方向、共享纠正预测与学习信用的具体缺口；不能把方向缺失和坐标代价合并成未经验证的唯一根因，
也不默认新Writer、续训、scale／rank／LR／阈值扫描。跨轮结论见findings§93。
原件在`runs/analysis/native_correction_writer_20260913/acquisition_audit/ray_*`：完整readout、四原始统计、
summary／task等权分组／decision、registration及源码／log／exit全部保留。下方旧阶段措辞均不恢复执行。
...
当时向Owner提出独立frame_set是否为持续硬门槛的疑问；2026-09-14已明确保留这一要求，见顶部。整体goal未完成。
...
### 当时待明确的后续方法选择口径（2026-09-14已澄清）

当时goal要求可重复有益视频增量及最终内容／顺序因果确认；已完成实验另预注册了独立训练ordered相对
全帧frame_set的优势。后者不是固定模型的顺序干预，也不是单帧或语言参照。后续是否把“胜过独立训练的
全帧无序方法”设为持续硬门槛，当时尚待Owner明确；2026-09-14已明确保留，最新边界见顶部。
不重判任何已关闭实验，不选checkpoint或提前运行最终controls。目标及其它科学合同保持。
本次同时核对findings§68的73/75任务原件索引与现有sampler；没有新增支持恢复95-task的证据，不重复该审计。
该疑问当时未阻塞与它独立的机制诊断；已完成的source输入诊断沿用当时科学边界，未据未回答的问题放宽方法资格。
