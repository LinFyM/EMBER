    ...
    *,
    ...
    for visit in range(visits):
        for ordinal in fit:
            ...
            cost = sum(
                _frame_count(by_ordinal[ordinal].episode_lengths[demo], stride)
                for demo in demos
            )
            rows.append((cost, ordinal, visit))
    if len(rows) != total_task_visits:
    ...
    for group_index in torch.randperm(len(groups), generator=generator).tolist():
        ...
        schedule.extend((group[index][1], group[index][2]) for index in rank_order)
