...
from ember.writer.architecture import SEMANTIC_FACTOR_BASIS_WRITER_PARAMETER_COUNT
...
AS_WRITER_LAUNCH_SCHEMA = "ember_pi05_semantic_factor_basis_launch_v1"
...
def resolve_runtime(
    ...
    if args.mode == "formal" and config["formal_run"].get("status") != "sealed":
        raise WriterModelError(
            "formal AS-Writer config is not sealed from the live factor-basis profile"
        )
...
def inspect_feature_cache(*_args: Any, **_kwargs: Any) -> dict[str, Any]:
    ...
    raise WriterModelError(
        "pooled PI05 Writer feature caches are retired; factor-basis AS-Writer "
...
def writer_trainable_contract(
    ...
    if (
        not names
        or parameter_count != SEMANTIC_FACTOR_BASIS_WRITER_PARAMETER_COUNT
