...
## 当前任务：v5.2机制与SFT差距原因研究（2026-09-18）

1. 核对共享SFT与A900的实际函数、数据/优化合同及全部已有400行，定位错误视频优势集中在哪些任务。
2. 固定A900，在全部train24同一动作查询上测试：相同真实视频下改变Writer语言、完全固定LoRA，以及关闭Procedure调制。
   使用两条事先固定的donor视频，不选优、不更新权重；功能指标只定位接口，不代替闭环。
3. 对能直接复用官方执行器的关键冻结干预补小规模train96闭环，再报告支持和排除的解释与残余不确定性。
   不另开新架构、长训练、v5.2重训或Test；不把临时干预当作fresh删除模块的效果。
4. 按Owner最新约130–140及视频特异性的标准讨论后续；少量正常churn不作为单独否决理由。

预注册原件：`runs/analysis/v52_mechanism_audit_20260918/registration.json`。当前没有新训练goal。
