...
在B首个闭环分数产生前，仅按train24 exact language与BDDL登记以下唯一候选：

| 任务分组（global IDs） | 共同条件及保留的异质性 |
| --- | --- |
| 0, 2, 5, 28 | 黑碗到盘，不同来源关系与场景 |
| 4, 9, 20, 39 | 柜具／带门容器的内部、顶部与开合；包含取放、打开、关闭 |
| 7, 21, 25, 38 | 炉具周围的对象及空间关系；包含移入、移出、推动和双对象 |
| 12, 14, 15, 19 | 不同单个食品对象放入篮子 |
| 16, 18, 35, 37 | 指定容纳区域；篮子与书架、单对象与双对象 |
| 22, 29, 34, 36 | 酒瓶／杯子放到指定支撑物；不同目标位置及单／双目标 |

Task4顶抽屉初始已打开，task38炉子初始已开启；task39微波炉初始打开且目标要求放杯后关门，
不得误写为4需要开抽屉或38需要开炉。六组精确覆盖train24各一次，组内语义纯度不完全相同。
每轮沿用B的原task permutation，按各组首次出现的顺序输出六组，组内也保留原置换顺序；
各task仍取本轮同一个occurrence事件。该重分组不可避免改变轮内更新位置与Adam历史，不能单独归因梯度冲突。
候选分组及1200步列表保存在`runs/analysis/v52_return_20260915/cooccurrence_candidate.json`，
进入C前仍须满足B获取证据条件；候选登记不构成C启动或结果。
获取条件满足后，B/C可在各自单节点并行完成剩余预注册节点：两臂模型相同，仅任务共现不同，
各自仍必须在一个节点的完整correct400及train96完成后才续下一段。物理GPU数可不同，保持四任务等权SUM与全局更新语义。
...
本次准入顺序为A历史复核→B现行基线→B具备获取后开放C单变量对照，不同时启动多个架构。
满足C前提后可并行完成同一模型的两种任务分组，完整保留B与C各自300/600/900/1200的证据。
