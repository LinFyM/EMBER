"""Authorized non-held LIBERO task roles and task-level folds."""
...
def _folds(protocol: Mapping[str, Any]) -> dict[int, tuple[int, ...]]:
    ...
    if set(folds) != set(range(5)):
        raise MetaProtocolError("meta-task folds must be 0..4")
    return folds
...
def load_meta_protocol(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise MetaProtocolError("meta-task protocol changed")
...
def meta_task_split(
    ...
    if selected not in folds:
        raise MetaProtocolError("held-out meta fold is outside 0..4")
