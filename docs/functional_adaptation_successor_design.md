...
状态：2026-08-21 **sealed predecessor design，不再拥有implementation authority**；统一fingerprint上的flow/probe/exact Decoder与随后
...
中期架构复核并最终采用`docs/event_conditioned_policy_compiler_design.md`所定义的EMBER-ECP。本文只定义已封存代码与
实验的历史authority，不据此启动formal训练。旧LMMPC、V6、LPCP、GOMQ与Expert-Manifold细节只从
