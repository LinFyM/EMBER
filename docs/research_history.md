...
query自己的state、同一完整LoRA和真实FM消费者仍不能单独证明跨初态规则已被学会。
旧24-task的165/147有限正增及2100的158/159、后来36-task cross辅助六节点较好、当前C0纯跨episode均保留，
不跨合同归因反转、不使用最终顺序controls修改设计。定向复读旧实际sampler/FM/VJP和既有匹配报告，没有新增实验。
本条配对风险理论线收束，未形成新的GPU候选，不恢复旧辅助/Reader/LocalField或另派同类公式研究；
整体目标未完成，精确执行状态以progress为准。
...
9月Video Functional及纠正获取路线，并保留近期信用诊断和撤回flow的边界；不是重跑旧实验或证明统一根因。
[findings§156](../findings.md#156-长期推进错误的回顾与跨会话决策约束2026-09-26)补充代表链、现存原件、
适用范围与新主讨论承诺：停止线须累计约束主假设，局部正例不自动组成端到端成功，架构基础可据证据撤换，
强参照不随失败下移，新失败架构不自动成为下一底座，重构仍需完整特征/算子/梯度/部署原则。
v5.2/GOMQ及privileged正例继续保留；未运行的Natural Program、短预算和有效科学阴性分别记录。
稳定纪律同步Owner要求§3，具体下一投入约束进入task_plan，实施范围和复核边界进入progress。
Reader仍仅在既定有界工程范围内，未因这次回顾新增实验或激活自动训练/蒸馏链。

Owner随后明确禁止近似重复旧失败架构/训练。补查Video Functional完整共同学习图、VL后继和实际梯度源码后，
纠正只强调固定E native诊断的比较不足：联合表示/执行reader并非新机制。对应原件和算子差异见
[机制分析§6.1](analyses/feature_to_operator_mechanism_20260926.md#61-最近似历史的完整比较与本次降级)。
主讨论下调当前Reader正式学习优先级；原有界工程收尾，630学习保持未激活，不因工程完成自动投入。
...
findings§156保存目标漂移、可达/学习/迁移混淆、局部数学归因、视频有益性、历史证据更新、比较口径、
能力保持及执行组织等十类错误与具体证据；稳定约束进入current_owner_requirements，当前工作顺序进入task_plan。
首帧学习是合理的有限对照，仍按既定合同执行；不因本轮批评将其宣布无价值或撤回，也不等其结束才研究架构/训练。
“MT-BC的失败”被澄清为不准确措辞：未完成某个评测回合不是已定位的技术缺陷，更不是EMBER的新研究目标。
本次无新科学结果、训练/评测派发或Sol指令。按Owner明确要求另写跨会话memory extension，仓库保持唯一项目authority。
...
[主讨论审计](analyses/language_content_path_audit_20260926.md)核对现有B/C真实计算：视频Core不直接携带q residual，
但图文E本身含语言；不能推成语义缺失或不可表达B。提出只增加一个共享标量内容项的待测构造，
给出包含现有B/C的参数条件及梯度式，保留语言捷径、信息未识别、旧视频正例和残差失败的竞争约束。
findings§152不声明根因或修复；下一步仅为隔离CPU构造验证，无GPU训练/真实输入/部署成绩。
具体派发、接手和后继裁决均以progress为准。

该CPU阶段随后完成，41a0f4f4以9行owner源码实现候选；主讨论审阅并独立复跑两个非退化构造测试。
完整76输出复现C/B，视频扰动及梯度作用通过；实际GPU/真实query/闭环为0。默认旧路径未变。
主讨论还从四臂原results复算6032行，确认C后期差距主要在额外Source损失，新增不持续少于B，
且630主要由Goal21的−32抵消其它七任务的+13；不能当整体视频学习失败。
据此登记[两臂内容路径学习](designs/language_content_path_causality_design.md)，各fresh630、单末点736行、
B630/Source新闭环参照。只作为保持/迁移与有益视频预测的检验，未产生新行为结果，实际派发见progress。
...
`01a0cd90-ebb7-77a1-a20b-a858825d2f66`执行具体实验。分工元数据在`bb13587e`登记推送，科学spec未变。
Queue message `01a0cd98-6793-7d13-8646-e54fef12f271`已接受；Sol随后在新turn明确接手并建立隔离树审阅未验证WIP。
派发要求修复/CPU/真实更新/恢复/物化/资源/Git检查通过后才开既定四臂，结束主动回报并停止新增实验；不联系旧Luna或旧主讨论。

主讨论同步读取相关完整历史论证和源码，复算旧配方四个400面板、近期16个400面板及局部接口160行。
新增的跨运行成功集合分解与边界见findings§134；可反驳机制框架写入task_plan，不构成追加实验。
此条记录时尚无四臂GPU结果。派发和复算小文件在study的`coordination/`，不重复仓库清理。

## 2026-09-23：授权新的主讨论session接续研究

仓库收尾后Owner要求由新的session陪同分析EMBER并尝试解决根因；短期可继续既定安排，具体实验可以派发其它session。
据此将四臂条件编译诊断登记为接续设计，保留既定科学spec，仅更新授权/分工/当前状态和已经完成的前置工作说明。
接手计划明确：先承接历史正负证据和WIP工程核验，以四臂缩小范围，再形成可反驳的机制预测与针对性干预，不能将矩阵排名当作根因。
本轮只完成交接材料，没有创建新任务、发送实验工作、运行GPU或产生新科研结果；新主讨论/执行者ID待实际接手登记。
...
屏蔽末P冻结correct/wrong9/7，对父8/8；18步后15/11/8（correct/other/wrong），原方法同18步15/10/9。
新修正相对匹配更新正确净0，仅一得一失，没有已验证的双目标修复，不批准正式训练。
共640条新比较条件、64个完整Writer梯度和18次新更新，复用160条A900原件；冗余FP32与分卡行单独保留。
O1200 OFF超时只补未完成4条；分卡/恢复与原进程退出如实登记。00:01本小时两节点作业全部结束，无Test/FT/RL。

2026-09-22夜间完成有界根因探索：旧MT-BC200补测102/400，新非峰成熟节点119–138，采点窗口只能解释部分差异。
共同五held任务旧MT-BC200/旧EMBER1500/新MT-BC300/C600为91/148/134/130；不作纯数据划分因果归因。
冻结BA/flow检查未支持简单因子相消或整条视频路径关闭；均值21.8倍衰减主要受少数夹爪分叉影响，中位数2.33倍。
保留同一C600父状态的54更新P/J/F（不更新/原损失/辅助改普通FM）训练侧72闭环39/46/44，M30044；
目标24各48条为18/23/23/21。F−J净−2，预登记明显收益未满足，没有可升级正式训练的已验证修复。
另补Spatial3两个wrong donor格：旧O1500两者均0，C600为40/45（各50），该特定donor混杂不能解释历史差异。
追加8个任务各自仅保留本task新梯度、但保持54个AdamW step的反事实：共同16条件原J成功7条，各独立副本共4条，
全零新梯度Z成功1条，J→独立副本R/G/L=3/1/4；不支持移除其它task新梯度即可修复。副本不是一套部署方法，
且步幅/Adam统计与两次裁剪也改变；不据此批准拆head、PCGrad或正式训练。
本轮852条新闭环、J/F共108次完整诊断更新；隔离检查另有48次task梯度事件、486个含零梯度optimizer step。
正式训练未开始；因子C600一条旧参考动作偏差保留为检查例外。
Owner澄清固定LoRA只是理想可达下界论证，不指定公共base课程；视频特异性服务于正确教学内容改善闭环。
完整原件在`/data0/user/ymdai/ember_runs/overnight_root_cause_20260922`，结论见findings§127；无正式训练授权。
...
代理撤回上述过早路线建议，保留全部实验事实及适用边界；未形成新的架构选择、实现或训练。
本次整理同步稳定要求、README阅读入口和当前状态，保留现有代码、正式原件及未合入/未提交工作树。
...
以同构块加深、保留原输出端，明确理论性质与未验证收益。没有新实验、held动作使用、Test或源码变更；新设计不构成运行授权。
跨轮结论见findings§115；当前授权与交付状态仍归progress。

2026-09-17对齐A按owner要求在当前训练自然完成后暂停：3000步／12000条件／252000queries，
每task500条件，2800／2900／3000完整恢复点、source冻结与原事件连续性审计通过；累计训练9.019h。
3000未启动LoRA物化或闭环评测，最新完整面板仍为2700；SFT早已结束，B未启动，相关进程均已退出。
原件为study的`A/training_audit_3000.json`与`A/owner_pause_completion_3000.json`；不把暂停记作方法完成或科学停止结论。
...
correct相对wrong／shuffled／reversed的差值区间均跨零，没有持续增强的特异性证据。
完整2096条新评测与72个最终workers通过，无本节点工程异常；事实与边界见findings§112及study的`A/continuation_readout.json`。
2400追加决定仅依据已完成主面板，见`A/node2100_primary_readout.json`；当前执行与后续授权归progress。
...
同日完成对齐source上的fresh A300：1200条件／25200queries、3422.504秒，完整correct400／train96为99／36，
...
能力获取回升，但99→88→140尚不构成相邻稳定，140也未超过145；两个验证task仍为零，此时无selected checkpoint或controls。
...
Owner随后单独授权固定新A900的视频特异性诊断，同日完整完成：correct140／other136／wrong116／no-video48／
shuffled128／reversed139，各400条。Wrong相对correct少24条、任务bootstrap95%CI[-12,-.5]pp，
相对other少20条、CI[-10.5,-.25]pp，支持有限内容特异性；打乱／倒序相对correct的区间均跨零，
没有建立正确时序的稳定优势。倒序总分139接近140，但churn71，不能推断输出完全不受顺序影响。
No-video是完整零LoRA，不能当language-only Writer。2000条新rollouts、48个最终workers及五个最终launcher均成功；
source／checkpoint／视频／初态／RNG／真实RGB重排审计通过，两次无rollout的GPU准入失败原件保留。
...
frame_set200 train other47/96（S/O/G/L=14/17/11/5、breadth17），100→20038→47为29/18/9、churn27/J=.51786、95%CI[-.05208,.25]；other→correct47→46为42/4/5、churn9/J=.82353、CI[-.07292,.05208]。main200other47与它同分，43/4/4、churn8/J=.84314、CI[-.05208,.05208]；200步训练两臂均未证明有序增量。
...
32组双相机拼图全部人工检查。奶油奶酪可见获取后续衔接和取放迟滞；其它任务同时出现错实例／错物、
抽屉子阶段停滞与组合目标失去。Long1/state0/F50入篮223步成立、361步失去；Long2/state12/F50开炉70步成立、120步失去，
这些完整谓词变化防止仅由稀疏图像宣称从未完成。具体接触／释放与部分相近物身份仍未识别。
