...
from ember.writer.architecture import CORE_PROGRAM_WRITER_PARAMETER_COUNT
...
AS_WRITER_LAUNCH_SCHEMA = "ember_pi05_core_program_as_writer_launch_v1"
...
def resolve_runtime(
    ...
    if args.mode == "formal" and config["formal_run"].get("status") != "sealed":
        raise WriterModelError(
            "formal AS-Writer config is not sealed from the live Core-Program profile"
        )
...
def writer_trainable_contract(
    ...
    if (
        not names
        or parameter_count != CORE_PROGRAM_WRITER_PARAMETER_COUNT
