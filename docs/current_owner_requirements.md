...
## EMBER能力目标的澄清（2026-09-22）

Owner以“Writer固定输出同rank的一套MT-BC LoRA”这个退化解说明EMBER理想模型类的可达性能下界；
正确视频应提供超过该退化解的能力空间。这不是要求先学习公共能力、预训练公共底座，再学习视频残差，
也不是指定分阶段课程。须研究整体端到端学习为何没有实现应有能力，以及怎样同时提高闭环表现和视频有效利用。
不可反复用rank差异或现实现的表达限制代替根因分析；若实现限制妨碍目标，它本身就是待验证、待解决的问题。
当前MT-BC对照只作能力测量参照，不据上述论证自行引入公共base或新的训练阶段。

视频特异性用于检验“正确教学内容→更合适的LoRA→更好的闭环执行”这条机制链，不是独立追求漂亮差值。
视频导致参数或动作发生变化仍不足以说明有效利用；须寻找任务相关信息的有益贡献，并让它服务于绝对能力提升。
不得通过故意损坏wrong表现、放大无益敏感性或牺牲correct能力来满足特异性目标。

## 单次覆盖重训与后继实验授权（2026-09-20）
