...
from ember.source_sft.sampler import SourceBaseBatchSampler
...
def _build_dataset(
    ...
    asset_validation = validate_runtime_assets(
        ...
        context=context,
        verify_weight_hash=config["runtime"]["asset_verification"] == "full_hash",
    )
    ...
    asset_validation["source_corpus"] = validate_source_files(
        ...
        manifest=manifest,
        context=context,
        verify_hashes=config["runtime"]["asset_verification"] == "full_hash" and not args.skip_data_sha,
    )
...
def _build_models(
    ...
    ema_policy = None
    if ema_enabled and context.is_main:
        ema_device = torch.device(config["runtime"]["ema_device"])
        ema_path = None if args.resume is None else args.resume / "ema_policy"
        ema_policy = (
            copy.deepcopy(policy).to(ema_device).eval()
            if ema_path is None
            else load_policy(ema_path, config, ema_device).eval()
        )
    ...
    optimizer = torch.optim.AdamW(
        ...
        foreach=config["runtime"]["optimizer_foreach"],
    )
...
def _build_loader(
    ...
    sampler = SourceBaseBatchSampler(
        runtime.dataset,
...
def _prepare_training(args: argparse.Namespace, context: DistributedContext) -> TrainingRuntime:
    ...
    if (config["data"].get("action_start_offset") != 1
            or config["data"].get("query_alignment") != "post_action_observation_future_control_v1"):
        raise Pi05SourceTrainingError("source queries must pair post-action observations with future actions")
