...
四视频32 fit/held41/38、64为41/39，均低于相同两视频A2参照43/41与44/45；没有新增task/action rows或更新数。
64两→四的R/G/L为fit37/4/7、held34/5/11。四视频相邻fit35/6/6、held32/7/6；Goal held仅保留2/4，Long fit两个1/50
互不相同、held两点均0。64跨视频Jaccard.702低于两视频.780。完整600新行和全部配对见four-video analysis，历史§169。

Goal/Long held functional升至.001330/.001127却没有闭环收益；更多fit视频均值与原两视频匹配均值已分开。该固定预算覆盖干预未获支持，
不继续增加同task视频或直接续训；下一对照回到闭环较高的两fit条件。更多meta task、过程/整策略表示和监督到行为仍是竞争解释，
不能把任何一个局部负结果直接命名为根因。同图clone/shared也未证明shared必然弱于clone，不能称本次已识别梯度冲突。
...
455--499 states；没有增加action rows总数或task数，也没有改变Panel-B评估。正式authority为`078a2e68`，新analysis根为
`runs/analysis/pi05_ecp_prw_samegraph_query_coverage_20260906/`。
...
## 175. 训练侧失败需区分动作阶段与任务难度，不能过早归因occupancy

固定训练轨迹显示完整Writer已有部分抓取、搬运与过程推进能力，但接触、抽屉选择、碰撞和最终放置都可能失效。Long93的后半过程
...
## 179. 两种初始化下的共享缺口接近，下一步可检验统一容量扩展

当前完整图同18tasks/128updates/实际18432queries的fully-random验证曲线为16/16/17/19（各80），component为17/17/20/16；
random96的breadth6到128缩为3，Long新增未保持，仍低于同prefix SFT24。固定两训练task为4/20，对component3/20与独立clones14/20。
初始化没有消除已定位的共同学习代价；不能由一个终点净增3或内部功能loss证明修复，也不能把negative扩展为函数类不可学习。

同图少任务与独立学习已有正行为证据，共享18在两种初始化下持续较弱。这为检验统一表示/读出容量扩展提供了依据，尚不能区分表示、
参数容量、梯度作用和优化，也不证明宽度不足就是根因。标准width128→256、heads4→8保留head dimension32及4blocks，现有代码直接支持，
全部Writer参数由4750208到15660800；它同时扩大P/Q与family heads，不拆出某一层的独立优势。最长87-frame、micro8两步profile已正常完成，
peak34.811GiB、train3.347秒，只有运行可行性意义。下一正式对照保持18tasks/原视频/128updates/实际query与optimizer，先以训练侧行为及
validation相邻曲线判定增量；没有增量时，需要新的定位证据才能继续同轴扩张。具体旧证据与本轮边界见历史§181。
