...
- main50/100/200/300所有train96及validation400 correct/other面板均完成；main300 train43/46、validation32/32。frame_set50/100/200全部面板完成，200 train46/47、validation69/61。frame_set训练按登记完成300，全部1,200条件/76,800queries与main匹配，完整checkpoint及四套LoRA banks保存。frame_set300 train correct48/96已完成，other在gpu01:6；validation correct在gpu01:4,5、other在gpu02:0,1，两组各6workers已就绪。frame_set200 other释放后，当前仅5张，无新增训练。
...
- main200 validation other=74/400，S/O/G/L=1/44/18/11、breadth7；100→20071→74为41/33/30、churn63/J=.39423、95%CI[-.0925,.09]。other→correct74→74为59/15/15、churn30/J=.66292、CI[-.0125,.015]；正确/换视频同分但仍存在成功集合更替。释放gpu02:6后已在该卡启动frame_set200 validation other，保持全局6张。
- frame_set200 train other=47/96，S/O/G/L=14/17/11/5、breadth17；100→20038→47为29/18/9、churn27/J=.51786、95%CI[-.05208,.25]。other→correct47→46为42/4/5、churn9/J=.82353、CI[-.07292,.05208]；main200other47与其同分，43/4/4、churn8/J=.84314、CI[-.05208,.05208]。200步训练两臂都未形成可信有序增量；main300 train other已在释放的gpu02:0接续。
- main300 train other=46/96，S/O/G/L=13/15/14/4、breadth20；200→30047→46为32/14/15、churn29/J=.52459、95%CI[-.10417,.08333]；other→correct46→43为38/5/8、churn13/J=.74510、CI[-.11458,.05208]。
- main300 validation other=32/400，S/O/G/L=0/17/11/4、breadth6；200→30074→32为22/10/52、churn62/J=.26190、95%CI[-.1925,-.0225]。other→correct32→32为23/9/9、churn18/J=.56098、CI[-.0225,.025]。两组同task教学视频都出现迁移退化，不能归为单组视频配对偶然。
- frame_set200 validation correct=69/400，S/O/G/L=2/41/15/11、breadth7；100→20073→69为37/32/36、churn68/J=.35238、95%CI[-.15,.0825]。main20074相对它为52/22/17、churn39/J=.57143、CI[-.025,.05]；200步正确视频仍没有可信有序增量，其换视频及300步匹配仍待完成。
- frame_set200/300续训段完整结束，耗时10470.90秒，最终300更新/76,800queries，峰值39.05GiB；300 checkpoint含writer、optimizer与两个rank状态，实际曝光匹配见analysis的`frame_set/step300/exposure_alignment.json`。最后材料生成前strg01个人用量842.6GiB/1TiB（最终checkpoint前），run56GiB；包含最终checkpoint及banks的预计额外峰值7GiB，总峰值849.6GiB。双节点live检查仅gpu02:6有本轮作业，加入gpu01:5后共2张。
- frame_set300验证两臂已分别采用两GPU×3replicas启动，沿用单个cost-balanced动态队列、固定400视频条件及官方评测；没有新增科学变量。启动前live双节点仅gpu01:6和gpu02:6有本轮作业，加4张目标卡后共6张；精确分配与命令脚本见analysis的`frame_set/step300/validation_launch.json`。
- frame_set200 validation other=61/400，S/O/G/L=1/33/13/14、breadth7；100→20074→61为31/30/43、churn73/J=.29808、95%CI[-.1675,.0525]。other→correct61→69为48/21/13、churn34/J=.58537、CI[-.015,.06]；main200other74相对它为51/23/10、churn33/J=.60714、CI[-.01,.0775]。200步主方案correct/other差额+5/+13仍未达可信增益资格。
- frame_set300 train correct=48/96，S/O/G/L=16/15/13/4、breadth19；200→30046→48为36/12/10、churn22/J=.62069、95%CI[-.09375,.13542]；main30043对它为38/5/10、churn15/J=.71698、CI[-.13542,.03125]。其最后train other已在gpu01:6接续，3workers就绪。
