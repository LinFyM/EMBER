...
def test_profile_is_authorized_before_formal_and_formal_opens_only_after_seal() -> None:
    initial = load_v6_prior_config()
    assert runtime_for_mode(initial, "mechanism-profile") == (1, (), 0)
    with pytest.raises(ExpertManifoldError, match="blocked by live gates"):
        runtime_for_mode(initial, "formal")
    formal = _formal_ready_config()
    assert runtime_for_mode(formal, "formal") == (25, (10, 25), 0)
