...
`docs/ember_pecs_falsification_card_20260823.md`。其local-effect held5虽由shared `43`提高到`58`，但direct retention、breadth与
Goal/Long均未过门；当前只授权加入完整fixed-noise denoising action/flow trajectory target复验一次。在该privileged oracle
通过前，不授权video effect predictor或恢复本节旧`q_V`路径。
