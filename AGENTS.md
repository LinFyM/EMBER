...
- 主agent应主动判断并行收益。任务能拆成独立交付、可同时推进且明显节省总耗时时，直接派发最少必要的subagents；
  不必等待owner逐次点名。整库整理中的设计记录、源码生命周期审计、存储审计是典型可分离工作；简单读取优先并行工具调用。
- 每个subagent必须有明确目标、输入、停止条件、写入范围与交付证据；写入代理使用独立worktree。主agent同时推进其它工作，
  统一承担科学判断、冲突处理、集成与比例验证，禁止多代理在同一worktree重叠写入或互相撤销改动。
- 不为形式并行派发重复方案、常规审查或无独立产出的研究；不把紧密耦合的架构取舍或所有权不明的删除分散给代理。
  一轮派发完成后按实际剩余工作重新判断，不自动追加代理波次。
