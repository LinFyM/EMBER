...
## 当前goal：先完成原因分析，再自主修改、正式训练并迭代改进

2026-09-10 CST Owner睡前明确扩大授权：“依然是先把之前要求的各种分析都做完”，完成后取消必须停在修改当前方法和正式启动下一次训练之前的限制，允许继续推进、迭代改进当前方法。本条覆盖此前的正式采纳/合入/训练停止边界，不取消分析先行要求。

目标是先查清当前Horizon相对旧v5.2/v6能力较弱、训练改善与未见任务闭环不一致及能力丢失的主要原因，完成有区分力的学习对照、候选复核、历史反证和未识别边界报告；随后依据证据修订当前方法与正式设计，集成必要实现，启动正式训练、闭环评测并持续迭代。以仓库既有validation8 strict single-checkpoint correct严格>145/400及相邻稳定、breadth、四suite/Goal/Long、同task换视频与最终视频因果资格作为科学裁决，不把报告完成、一次launch或单点提升当作最终完成。Test仍只在方法冻结后的既定最终合同下使用，不提前开放。

**当前active诊断计划：** [学习过程因果分析计划](docs/horizon_causal_learning_plan_20260909.md)。正式方法目前仍为`docs/horizon_relation_video_writer_design.md`与现有canonical实现；完成分析并由证据支持修正后，同步正式合同和实现，不另等Owner醒来批准。
...
5. **随后自主方法迭代：** 依据分析证据选择修改，更新正式设计/合同，完成隔离实现与适当验证后集成main并推送；从clean pushed detached运行面启动预登记正式学习与闭环评测，继续按实际获取、迁移、保持和全部科学资格迭代。正式采纳和下一次正式训练已获授权，不再设置阶段性人工批准停止点；旧v6等已暂停方案不因此机械重启。
