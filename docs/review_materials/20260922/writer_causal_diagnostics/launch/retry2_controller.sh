#!/usr/bin/env bash
# Fixed-phase retry controller.  Low-occupancy foreign GPUs are explicitly authorized for co-use.
set -uo pipefail

STUDY=/data0/user/ymdai/ember_runs/writer_causal_diagnostics_20260922
WORKER="$STUDY/launch/retry2_run_worker.sh"
RUNTIME=/data1/user/ymdai/projects/EMBER/.codex/tmp/writer-causal-runtime-7190bf07
PYTHON=/data1/user/ymdai/projects/EMBER/.venv/bin/python
START_EPOCH=$(date +%s)
DEADLINE_EPOCH=$((START_EPOCH + 7200))
D3_MIN_REMAINING=3900
SIGNAL=ember-writer-causal-diagnostics-20260922-retry2-complete

printf '{"started_epoch":%s,"deadline_epoch":%s,"wallclock_limit_seconds":7200,"d3_min_remaining_seconds":3900}\n' \
  "$START_EPOCH" "$DEADLINE_EPOCH" >"$STUDY/launch/retry2_controller_started.json"
PIDS=()

finish() {
  code="$1"
  printf '%s\n' "$code" >"$STUDY/launch/retry2_controller.exit"
...
start() {
  ...
  name="$3"
  shift 3
  if (( $(date +%s) >= DEADLINE_EPOCH )); then return 124; fi
...
run_finalize() {
  ...
  PYTHONPATH="$RUNTIME/src" "$PYTHON" "$RUNTIME/scripts/run_writer_causal_diagnostics.py" \
    ...
    >"$STUDY/launch/retry2_finalize.stdout.log" 2>"$STUDY/launch/retry2_finalize.stderr.log"
  code=$?
  printf '%s\n' "$code" >"$STUDY/launch/retry2_finalize.exit"
  return "$code"
}
...
if (( $(date +%s) + D3_MIN_REMAINING <= DEADLINE_EPOCH )); then
  ...
  printf '{"status":"not_started_insufficient_preregistered_time","checked_epoch":%s,"deadline_epoch":%s,"required_remaining_seconds":3900}\n' \
    "$(date +%s)" "$DEADLINE_EPOCH" >"$STUDY/launch/retry2_d3_not_started.json"
