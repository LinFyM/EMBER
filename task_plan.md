...
4. **短学习与行为（进行中：原96步全部结果已封存，单变量初始化fresh对照训练/评测正在运行）。** 预登记跨Spatial/Object/Goal及有实质专家容量Long的训练侧任务、跨episode角色、曝光和行为节点。
   原K1三点correct/other为4/6、6/5、4/4，对source4；96K4为6，均无Object/Goal。原完整图保持，针对近常量B与非恒定真实梯度检验native坐标初始化，见design§8.4。
   小面板用于定位与投入判断，不选择最终模型。未见基础行为前不默认投入约10小时长训练。
