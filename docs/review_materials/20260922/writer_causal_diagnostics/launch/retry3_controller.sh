...
WORKER="$STUDY/launch/retry3_run_worker.sh"
...
SIGNAL=ember-writer-causal-diagnostics-20260922-retry3-complete
printf '{"started_epoch":%s,"deadline_epoch":%s,"wallclock_limit_seconds":7200,"d3_min_remaining_seconds":3900}\n' "$START_EPOCH" "$DEADLINE_EPOCH" >"$STUDY/launch/retry3_controller_started.json"
PIDS=()
finish() { code="$1"; printf '%s\n' "$code" >"$STUDY/launch/retry3_controller.exit"; tmux wait-for -S "$SIGNAL" || true; exit "$code"; }
...
run_finalize() { status="$1"; PYTHONPATH="$RUNTIME/src" "$PYTHON" "$RUNTIME/scripts/run_writer_causal_diagnostics.py" --output "$STUDY" finalize --d3-status "$status" >"$STUDY/launch/retry3_finalize.stdout.log" 2>"$STUDY/launch/retry3_finalize.stderr.log"; code=$?; printf '%s\n' "$code" >"$STUDY/launch/retry3_finalize.exit"; return "$code"; }
