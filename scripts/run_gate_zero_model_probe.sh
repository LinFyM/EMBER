...
[[ "$mode" = base || "$mode" = adapter ]] || die "--mode must be base or adapter"
[[ "$task_id" = 3 || "$task_id" = 4 ]] || die "--task-id must be 3 or 4"
[[ "$micro_batch_size" =~ ^[1-9][0-9]*$ ]] || die "--micro-batch-size must be positive"
...
command=(
  "$PYTHON" -m ember.gate_zero_model_probe
  --config "$CONFIG"
...
[[ ! -e "$output_dir/probe_result.json" ]] || die "completed output already exists"
mkdir -p "$output_dir"
