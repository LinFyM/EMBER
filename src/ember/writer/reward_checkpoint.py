"""Cycle checkpoints for native probe-Value commitment."""
...
REWARD_CHECKPOINT_SCHEMA = (
    "ember_pi05_v6_lpcp_native_probe_value_commitment_checkpoint_v1"
)
...
def checkpoint_cycle(path: Path | None) -> int:
    ...
    if match is None or path.parent.name != "checkpoints":
        raise WriterModelError(
            "native probe-Value resume path is not a cycle checkpoint"
        )
    ...
    *,
    ...
    if context.is_main:
        ...
        if partial.exists() or final.exists():
            raise WriterModelError("native probe-Value checkpoint already exists")
        partial.mkdir()
    barrier(context)
    torch.save(
        {
...
def load_reward_checkpoint(
    *,
    ...
    if (
        ...
        raise WriterModelError("native probe-Value checkpoint manifest changed")
    ...
    if (
        ...
        raise WriterModelError("native probe-Value checkpoint cursor changed")
