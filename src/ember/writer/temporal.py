...
class LayerwiseProbeProcedureConditioner(torch.nn.Module):
    """Use one causal video controller to pool layer/rank probe changes."""
    ...
    def __init__(self, *, heads: int = 8, blocks: int = 1) -> None:
        super().__init__()
        ...
        self.context_norm = RMSNorm(self.WIDTH)
        self.delta_norm = RMSNorm(self.WIDTH)

    def forward(
        self,
        layer_rank_probe: torch.Tensor,
        frame_indices: torch.Tensor,
        video_offsets: tuple[int, ...],
    ) -> torch.Tensor:
        if (
            layer_rank_probe.ndim != 4
            or layer_rank_probe.shape[1:]
            != (self.EXPERT_LAYERS, self.RANK, self.WIDTH)
            or frame_indices.shape != (layer_rank_probe.shape[0],)
            or frame_indices.dtype != torch.long
            or frame_indices.device != layer_rank_probe.device
            or len(video_offsets) < 2
            or video_offsets[0] != 0
            or video_offsets[-1] != layer_rank_probe.shape[0]
            or any(right <= left for left, right in zip(video_offsets, video_offsets[1:]))
        ):
            raise VariableEpisodeInputError("invalid layerwise probe video batch")
        lengths = tuple(
            right - left for left, right in zip(video_offsets, video_offsets[1:])
        )
        maximum = max(lengths)
        videos = len(lengths)
        packed = layer_rank_probe.new_zeros(
            videos,
            maximum,
            self.EXPERT_LAYERS,
            self.RANK,
        ...
        for video, (left, right) in enumerate(
            zip(video_offsets[:-1], video_offsets[1:], strict=True)
        ):
            ordinals = frame_indices[left:right]
            values = layer_rank_probe[left:right]
            delta = torch.cat(
                (torch.zeros_like(values[:1]), values[1:] - values[:-1]),
                dim=0,
            )
            length = right - left
            packed[video, :length] = delta
            positions[video, :length] = ordinals
            valid[video, :length] = True

        temporal_summary = packed.mean(dim=(2, 3))
        contextual = self.temporal(temporal_summary, positions, valid)
        ...
        final_context = self.context_norm(
            contextual[torch.arange(videos, device=contextual.device), final_indices]
        )
        slot_delta = packed.reshape(
            videos,
            maximum,
        ...
        final = (weights[..., None] * slot_delta).sum(dim=1).reshape(
            videos,
            self.EXPERT_LAYERS,
            self.RANK,
            self.WIDTH,
        )
        expert = final.reshape(
            videos, self.EXPERT_LAYERS * self.RANK, self.WIDTH
        )
        result = torch.cat((expert, final[:, 0], final[:, -1]), dim=1)
        if result.shape != (videos, self.QUERY_COUNT, self.WIDTH):
            raise VariableEpisodeInputError("layerwise probe policy topology changed")
        return result
...
class ProcedureSlotReader(torch.nn.Module):
    ...
    def read_normalized_core(
        ...
        if query_condition is not None and query_condition.shape != routing.shape:
            raise VariableEpisodeInputError("Procedure query condition changed shape")
