...
**当前active为[冻结局部动作生成诊断](docs/frozen_local_action_decode_audit.md)。**
时间对齐与冻结正例复核均已关闭；当前检验新机制的具体前提，整体goal保持，无新Writer训练。
...
3. **理论边界复核已形成：**[视频信息与可识别性](docs/video_information_identifiability.md)核对了全帧frame_set、
   ...
   **当前诊断：**旧局部FM小幅优势是否成为从纯噪声还原动作的知识。先计算无视频Gaussian去噪参照，
   再冻结旧两臂step200，在原train24留出片段上以固定10步/8noise生成，比较任务均值及frame_set。
   不通过则停止现成局部动作中间量的提案依据；通过也只继续机制推导，不自动训练新Writer。
