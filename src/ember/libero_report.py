...
table {{ width:100%; border-collapse:collapse; min-width:1050px }} th,td {{ text-align:left;
...
<div class="table-wrap"><table><thead><tr><th>#</th><th>Split</th><th>Scene</th><th>Task</th>
  <th>Instruction</th><th>Demos</th><th>Frames</th><th>Audit</th></tr></thead><tbody id="rows"></tbody></table></div>
...
function draw() {{
  ...
  const shown=tasks.filter(t=>(!split.value||t.split===split.value)&&(!scene.value||t.scene===scene.value)&&
    (!needle||(t.task_name+' '+t.language).toLowerCase().includes(needle)));
  document.querySelector('#rows').innerHTML=shown.map(t=>`<tr><td class="num">${{t.task_index}}</td>
