...
2026-09-09 Owner恢复证据驱动自主推进，并明确分析实验/架构内部拆解直接执行，正式改模和训练之前先充分分析；休息期间继续自主推进不违反科学精神的工作。换卡、常规调度与小问题不再逐项询问。
目标仍为K1 Writer的初次完整LoRA具有稳定跨任务闭环能力；唯一正式性能与资格合同不变。旧报告交付阶段的暂停已被覆盖，但旧训练和24-task草稿不自动恢复。

当前首要任务是把“任务内获取、跨任务保持不足”进一步变成可区分的机制问题，避免因为一种修改容易实现就先跑它。
active design §8.2.4登记新增历史反证、第一项行为诊断、候选修正的顺序及结果分支；本轮不同时改变任务组织、语言内容路径和输出共享。

1. 完成最近等价历史核查：Target-Owned原生rank共享的旧non-pass、旧content/routing实现与当时对照边界；把它们纳入原因排序。
2. 并行开展冻结训练侧内部分析：24tasks×teacher46×固定16queries，正常输入，不更新参数；分解P4、语言query与compiler cross/self/FFN输出的局部FM敏感性，不当成闭环或全图因果证明。行为证据安排为冻结原200/600、固定任务/状态/视频/RNG的行为轨迹诊断。复用现有LoRA，不训练、不重新挑checkpoint；先定位目标获取、接触/抓取、后续放置/组合与已获得能力丢失的可观察阶段。
3. 根据诊断决定是否存在可直接修正的执行/监督支持问题；若只能定位行为而不能定责Writer模块，明确保留这种不确定性，再选择一次有新机制依据的fresh对照。
4. 架构候选优先审视compiler中“路由query直接成为输出内容”的接口；native rank共享和24-task组织后置，不把旧头部方案当新发现。具体替换公式、初始化/学习合同与100/200等节点须在实施前登记，不能一边改多个接口一边归因。
5. 新学习每段约一小时，节点按真实吞吐预登记；有实质能力则继续相邻稳定性，连续无有效行为增益则分析已检验假设。正式选择始终使用single checkpoint canonical400，train侧小面板/轨迹与内部指标不选模型；RL、K>1和最终controls按原合同后置。

已完成：只读原因报告与本轮近等价历史/参数化分析。未完成：新行为诊断、修正实施及其行为验证。当前没有新实验分数或已证实修复。
