...
def test_ap_adr_full24_config_seals_architecture_and_information_wall() -> None:
    config = load_writer_config(CONFIG)
    writer = config["writer"]
    assert writer["architecture"] == (
        "pi05_amplitude_preserving_asymmetric_dual_read_v1"
    )
    ...
    assert writer["program_value_path"].startswith("raw_A_E_D")
    ...
    assert writer["program_coordinate_reader"].startswith("38x16")
    assert writer["reader_softmax"].startswith("independent")
...
def test_ap_adr_launch_contract_records_raw_mean_collectives_not_ddp_accumulation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = load_writer_config(CONFIG)
    context = DistributedContext(
...
def test_retired_writer_configs_are_not_active() -> None:
    for name in (
        ...
        "pi05_as_writer_unified_causal_program_full24_decay400_v1.json",
        "pi05_as_writer_unified_causal_program_serial4_exposurematched_v1.json",
    ):
