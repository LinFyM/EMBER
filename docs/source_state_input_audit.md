...
**已完整完成并关闭；预登记状态补全前提未通过。** 原合同保留，完成结果见第5节，不从本文件恢复运行。
...
clean pushed `6fa6177f`冻结运行，三臂各384位置完整exit0。每臂约311.18秒、allocated峰值9.88GiB；
固定位置、动作区间和noise逐行配对，任务动作均值分数与先前保存的384位置重算一致。无新训练或rollout。

| 输入臂 | 生成均值MSE |
| --- | ---: |
| state-free | .13672735 |
| task mean state | .14173378 |
| true current state | .12469236 |
| task action mean | .25059744 |

state-free−true差额+.01203498，task-cluster95%CI[−.00247592,+.02751481]，16task正／8负；
mean-state−true为+.01704142、CI[+.00965492,+.02384471]，21正／3负；
action-mean−true为+.12590507、CI[+.09535753,+.15141732]，23正／1负。
真实state相对state-free的suite差额S/O/G/L为+.01830/−.00522/+.01907/+.01598。
第一项区间跨零，三个参照未同时通过。因此停止以当前证据直接增加状态补全模块的提案依据，不扫描或追加生成。
真实state对均值state的正差额保留，不能将本项写成“状态没有任何作用”。

另作明确标注的描述性追踪：action-mean−state-free差额+.11387009，使用同一task bootstrap的区间
[+.08481614,+.13969462]，22task正／2负。它不改变预登记的non-pass，但提供了新的正事实：
**合法state-free RGB＋语言输入下，冻结source完整采样具有优于该任务均值参照的动作生成能力。**
这不是视觉或顺序因果检验；不证明t=1的单步H、训练后的Meta/E或当前Compiler已有效保留这项能力。
也不与先前四帧局部逆动作头构成同输入／同任务形式的直接模型对照。

本项观察发生在记录的演示状态上，不等于新初始化闭环成功。后续先区分“原生完整采样有知识”与
“单步响应已是充分过程教师”，核对旧Action-Forecast的实际失败假设；不自动增加denoise阶段／层位扫描或重训Writer。
三个原始JSON、samples、全task／suite／描述性动作分量、`summary.json`、`READOUT.md`和`launch_contract.json`
保留于`runs/analysis/source_state_input_20260913/`。一次性入口从active scripts退役，冻结源码与数据原件保留。
