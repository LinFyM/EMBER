...
本文最初只派发末节的CPU构造验证；该阶段现已完成。任何后继GPU范围须读独立设计及progress，本文不自动授权训练。
...
重新逐行核对原Source及B/C六节点held/seen共6032行（task/state、env/policy seed及共同replan前缀、B/C视频映射）：
在630节点，B的Source-relative retained/gained/lost为48/76/10，C为24/81/34；C总分少19，
却多获得5、额外丢失24。840与1050的C也有更多新成功而更多损失；1260新增几乎相同。
630的Goal21贡献−32/50，其余七任务合计+13/350。因此不能把总差描述成整体获取能力普遍下降，
也不能删去Goal21后宣称视频有效。seen64的C−B六节点为+7/−10/−2/−3/−6/+1，没有统一劣势。
原件为conditional-compilation study `evaluation/{Source,B_language_*,C_video_fm_*}_{held,seen}/results.json`；
复算索引暂存本仓`.codex/tmp/language_content_path_cpu_20260926/paired_history_recheck.json`。
...
可能在共享更新及未见任务迁移时改变有效的任务内容；直接内容路径可能减少额外能力损失，同时保留video分支新获得的能力。
这不是“视频整体学不会”的假说；主预测必须同时涉及Source-relative损失减少与新增能力保留。
若总分改善只来自其它能力增加，而原有能力损失未减少，只能登记经验收益，不能据此确认本解释。
...
- H_path的主预测还要求相对原C减少Source成功集合上的损失，并保住原C在Source失败集合的新成功；
  单看总数无法签收这个解释。使用同批Source，旧集合只作历史诊断，不拼成新配对。
...
独立实现41a0f4f4，9行owner源码变化，默认旧行为/权重键不变。非零heads的76输出中，
a=0对C和指定构造对B的最大绝对差均0；视频扰动输出差 .44040，padding余项6.41e−7；
a梯度及推导内积均 .07022614。Sol针对性2项及既有10项通过；主讨论审阅源码/测试并独立复跑2项通过。
只有合成Core/Procedure中间表示，未加载source、真实视频或新增环境步；该边界保留。

主讨论接受构造正确性，并登记[两臂匹配学习](../designs/language_content_path_causality_design.md)：
新C0/Cplus各fresh630，已有B630/Source作为新闭环参照；首阶段单末点736条，先检验保持与新增能否同时改善。
该学习预算基于历史真实吞吐及固定阶段需要，不由CPU通过自动推出。正式派发/实现/资源只看progress及新合同。
