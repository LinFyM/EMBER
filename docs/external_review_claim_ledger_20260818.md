...
状态：2026-08-19 completed and sealed。本文是`docs/external_review_20260818.md`的逐项执行账本；专家报告中的每个
...
| A1.02 | 当前信息墙与source freeze基本正确 | code/scientific fact | `confirmed-code/artifact`；A/B/C全部真实functional backward的source nonzero gradient tensor均为0 |
| A1.03 | action-hidden视频影响内部表示 | experiment fact | `confirmed-artifact/closed-loop`；A/B/C/F5均显著受部分视频controls影响，C/F5相对no-video净+63/+60；影响方向与suite分布并不等于完整高层过程 |
...
| A2.02 | patch grounding与interaction projection因此无functional credit | code fact | `confirmed-code/artifact`；A/B在macro1和macro25均为no-gradient，C移除第二次detach后macro1首次出现nonzero finite gradient |
...
| A2.05 | optimizer/现有测试没有发现intended fresh modules无梯度 | coverage gap | `confirmed-and-closed`；本轮新增projected-output局部credit、source freeze和intended-path完整分组regression，真实backward evidence另行公开 |
...
| A3.02 | current123相对143为23 gains/43 losses | experiment fact | `confirmed-artifact`；400行remote evidence见G1 |
| A3.03 | current123相对151为23 gains/51 losses | experiment fact | `confirmed-artifact`；400行remote evidence见G1 |
...
| B1.02 | video应提供状态变化、先后关系、阶段、因果和nuisance过滤 | method boundary | `accepted as required role, only partially realized`；A使用内容与粗方向却失败于shuffle，B/C no-Text controls进一步检验正确中间顺序 |
...
| B2.02 | Core聚合全视频语义，容易受task/static cue主导 | code + hypothesis | `confirmed-code, mixed performance evidence`；wrong/no-video下降反驳纯static/task identity，但shuffle不降说明正确中间过程并非必要，Core独立贡献仍未隔离 |
...
| B3.01 | Core适合task语义但可能成为identity/static shortcut | hypothesis | `mixed`；cross-suite wrong/no-video显著更差，但correct不优于shuffle，static/task shortcut空间仍存在且未证明主导 |
| B3.02 | Procedure有真实顺序敏感性 | internal experiment fact | `confirmed-artifact but not useful-direction evidence`；A correct与shuffle相当、低于keep-first shuffle，正确中间顺序未成为闭环必要条件 |
| B3.03 | v4 shuffled优于correct警告order sensitivity可学错shortcut | historical fact | `confirmed-history`；F0b必须看方向而非hidden margin |
| B3.04 | 当前最早可证明失败接口是fresh projection→Core/Procedure | causal localization | `confirmed and repaired, refuted as dominant performance root`；修复显著改善reverse/wrong资格，却只净+6且不改善breadth/stability，未解接口后移到learned Program方向质量与shared retention |
...
| B4.03 | nonzero video可作carrier、language gate决定task方向的条件旁路存在 | structural possibility | `constrained but not eliminated`；wrong nonconstant video比correct低42、no-video低75，反驳任意carrier充分；shuffle反例仍显示过程可被旁路 |
...
| B8.03 | expert-demo occupancy与generated-policy occupancy不同 | distribution fact | `confirmed by contract`；F2不支持简单self-occupancy divergence解释，缺合法correctness reference使更广因果仍不可定向 |
| B9.01 | 流水线有真实新机制，但当前不能声称高层过程理解 | overall verdict | `confirmed by closed loop`；当前能证明视频内容与部分顺序线索有用，尚不能证明跨task、跨初始化的完整高层过程Program |
...
| E4.01 | 对仓库六项解释的修正需进入后续措辞 | reporting requirement | `completed`；最终专家报告逐项采用C4/E4的限定措辞，不再把task drift、Procedure趋同、functional mismatch、mean/Adam或address preservation写成已证根因 |
...
| F0a.04 | 覆盖Text/Action Meta-LoRA、Core、Procedure、memory、Reader、K-set、M2P、8 heads | coverage requirement | `completed for A/B/C`；A的Text与Action及全部后端组有credit，B/C不存在Text/VL且Action及全部后端组有nonzero finite gradient |
...
| F1.01 | 仅移除fresh输出detach的matched干预 | training recommendation | `completed through A/B/C attribution`；B先隔离no-Text，C相对B只恢复三项projected output credit；C macro25=`110` |
| F1.02 | 保持frozen hidden detach、拓扑、rank16、data/B20/optimizer/LR/seed/K | single-variable contract | `confirmed by run contracts`；B/C均fresh world6，除目标autograd连接外合同一致 |
| F1.03 | source零梯度、fresh modules非零梯度测试 | regression requirement | `completed`；C macro1 grounding/interaction全组nonzero finite，source policy gradient tensors=0，见`gradient_credit_evidence.json` |
| F1.04 | 专家root-cause/method/negative thresholds | advisory thresholds | `negative for absolute/stability root-cause and method pass`；B→C仅+6、p=`.45138`且breadth不增，C25→50仍33 lost、breadth6→4；但controls支持其为视频方向credit根因，不再围绕detach调参 |
| F1.05 | 修复后视频controls应改善 | causal qualification | `partially supported`；B→C correct/wrong/reverse=`104/65/96→110/54/69`，reverse由不显著+8变显著+41；shuffle margin`21→19`近似不变，keep-first`14→17`，same retention`83.65→87.27%`仍低于90%，且C的优势suite集中 |
...
| F5.04 | conflict-safe更新不能以牺牲视频过程资格换稳定 | additional closed-loop check | `mixed negative`；PCGrad保持wrong/reverse/no-video强区别且suite分布更广，但correct-keep-first仅+2且不显著，对照C的+17；它强化全局箭头方向、弱化中间阶段顺序，没有形成合格稳定方法 |
...
| G2 | current macro25全部video controls与不同K4 sets | `completed`；`video_causality_evidence.json`公开A的7面板，`b/c/f5_video_causality_evidence.json`进一步公开三个no-Text arm的同口径面板、paired统计、逐行success与video/RNG reference |
| G3 | intended modules实际gradient表与首次非零macro | `completed`；A/B追加matched macro1真实backward，所有非detach upstream在第一次更新后首次非零，旧grounding/interaction在macro1/25均无gradient；C修复组在macro1首次非零，所有source gradients为0 |
| G4 | 每个formal run的commit/dirty diff/source/config/checkpoint/metrics provenance | `completed for all retained review evidence`；training/eval commit差异、schema、manifest、bytes和contract均显式保留；依owner效率边界不新增逐文件SHA/MD5扫描 |
