"""Cycle checkpoints for the gradient-open semantic commitment."""
...
REWARD_CHECKPOINT_SCHEMA = (
    "ember_pi05_v6_lpcp_gradient_open_semantic_commitment_checkpoint_v1"
)
REWARD_DEPLOYMENT_KIND = (
    "v6_lpcp_gradient_open_semantic_commitment_cycle_checkpoint"
)
...
def checkpoint_cycle(path: Path | None) -> int:
    ...
    if match is None or path.parent.name != "checkpoints":
        raise WriterModelError("gradient-open resume path is not a cycle checkpoint")
    ...
    *,
    ...
    if context.is_main:
        ...
        if partial.exists() or final.exists():
            raise WriterModelError("gradient-open checkpoint already exists")
        partial.mkdir()
    barrier(context)
    torch.save(
        {
...
def load_reward_checkpoint(
    *,
    ...
    if (
        ...
        raise WriterModelError("gradient-open checkpoint manifest changed")
    ...
    if (
        ...
        raise WriterModelError("gradient-open checkpoint cursor changed")
