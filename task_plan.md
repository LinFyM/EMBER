...
2. **进行中：现行合同fresh基线。** 保留v5.2视频Core、Procedure与完整A/B能力路径，明确迁移双相机、完整50-horizon learned read、
   严格跨episode；300／600／900为validation97／105／77（均/400）、train35／49／50（均/96）。
   600→900验证保留55、新增22、丢失50；train保留34、新增16、丢失15。已精确续训最后的1200节点，检验交换与退化是否持续，不扩大预算。
3. **进行中：一个任务共现对照。** B300已建立可辨认train获取，仅改变同一train24事件的四task共现，保持曝光、query数、更新和学习率一致。
   C300为validation91/400、train28/96，分别低于B300的97和35，跨臂任务配对差额区间均跨零，尚无改善证据；C已精确续训600。
   C与继续中的B在各自单节点并行；两臂各自仍在当前完整节点面板完成后才续下一段，按既定曲线检验保持。
