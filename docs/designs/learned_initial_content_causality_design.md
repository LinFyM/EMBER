...
2. C0在held两种正确视频稳定占优，而seen没有S0整体学习失败：支持这个预算内首帧以外内容提供可迁移能力。
   仍不能区分场景覆盖与操作关系，也不证明超过语言或MT-BC；后续须解释哪些视频知识能够改善EMBER的参数生成与执行能力，
...
## 7. 完成裁决（2026-09-26；本批不再执行）

448条、24full、全T+1及8阶段退出均由主讨论独立核验，完整事件流/LR、2520事件与70560查询匹配。
C0/S0 correct=27/22、other=27/17、seen=26/22；主要correct差的task-bootstrap95%区间[-3.75,17.5]pp。
结果及配对集合见findings§159。正式来源4f6c75e4；两个held-other面板采用仅CPU验收修正b9e15410，
原bank/有效行未重算。完整进程计量4.669167 GPUh，在9上限内。

已退出训练检查发现S0 Action Meta到450才首次非零，C0从3开始；105至420的四个checkpoint中相关过程读出/调制仍为零。
首帧广播与零初始化读出、中心化、零调制共同存在精确算术不动子空间，完整推导及CPU核对见
[机制分析§11](../analyses/feature_to_operator_mechanism_20260926.md#11-首帧学习结果与可证明的初始化交互)。
这是本对照的学习机制限制，不是实施不合约，也不是完整视频C0的统一根因。
因此不能把小面板胜出当作信息必要或顺序知识证明；不追加S0修复/续训/400/精度调查。
后继方法由progress中的新登记限定，不能从本文件的旧“继续”恢复本批。
