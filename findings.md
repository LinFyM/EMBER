...
## 37. Compiler检索差异被共用位移压缩，但净学习与保持须分开（2026-09-10）

已有all/local_h_read × init7/init11 × 200/400八个checkpoint的小预算CPU选参诊断定位到第一Compiler的“语言位移相加→LayerNorm→Q投影”：all的608个槽检索差异幅度仅为同权重、位移置零代数反事实的3.5%–4.6%，同task query cosine约.9985–.9992；候选实际约.044–.055。语言位移以共同方向为主，显式affine bias本身不足以解释该程度；all在200→400的Q及共用静态语言读出漂移也明显更大。它支持具体查询组织机制，不证明真实attention饱和、选错帧、最终代码坍缩或该机制独自导致全部行为分差；残差、后续Compiler与独立D仍可能补偿。

Compiler两个候选200→400均有真实净学习：init7验证108→126、train40→56；init11验证104→119、train44→61。相邻验证仍分别丢26/37，保持未修复；两个节点不足以证明平台、已穷尽或再训无效。四条完整Compiler日志均未实际触发阈值1的全局clip，故裁剪不是该四臂差异的实测原因；各臂末50步FM接近而行为不同，不能用平均拟合排序保持。停止新增完整训练来自Owner授权上限，并非科学平台结论。

共享200的固定32条件×38target几何又提供不同接口的诊断：生成B全部第一奇异方向能量>99.9979%，BA>99.9896%，而D_B本身仅38.54%–74.46%。因此形式rank16未被硬降，但实际代码经过共同字典后近乎只使用一个输出方向；不能说共享字典在数学上只能rank1。独立all的B没有这种程度的集中。该结果尚不能分开C、U_B/GELU与字典使用的共同适应，也不能替代400或闭环；绑定效应只适用于当前同target跨rank A/B两侧D的实际干预，不外推全部共享。完整推导、原件、竞争解释与未测设计边界均在[当前学习报告](docs/horizon_causal_language_learning_20260909.md)“结构与学习机制深化”节，原件`causal_learning_20260909/mechanism/`。

共享200全部行为面板完成后，validation为115对all103（R/G/L77/38/26、breadth均7），train96却32对41。因此不能把近单方向输出直接判为迁移障碍或工程缺陷；实际证据显示绑定改变了早期拟合与泛化的权衡。400与本run自身保持仍待完成，不能从早期净增宣布稳定收益。
