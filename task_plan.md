...
1. **工作理论与近等价核对已形成，继续深化方法。** [条件控制算子理论](docs/temporal_control_compilation_theory.md)
   联合解释输入内容、操作关系、执行知识、参数生成和获取／保持，区分事实与假设；新Writer尚未选定。
2. **进行中：检验固定G自身的闭环价值。** 恢复原注册480episodes，以决定是否继续把真纠正获取作为主要修复依据。
   之后从整体解释选定有实质依据的方法，不默认沿用当前纠正目标／Compiler，也不自动重启旧Writer。
   将比较条件、可解释结果分支、信息墙与有界学习／闭环节点登记为唯一active design，再实现和验证。
3. 依证据自主推进实现、真实吞吐检查、正式学习及匹配闭环；报告能力、ordered−frame_set、breadth、churn和相邻保持。
   负结果先更新主假设和投入判断，停止近等价重复；有益候选继续验证跨视频与冻结后的因果controls。
4. 仅在完整目标证据成立时完成goal；局部正例、surrogate、单个峰值、代码或理论文档完成均不算科学目标达成。

当前唯一active design为既有真实纠正闭环诊断，按原采样／门槛恢复；使用现有显式task-subset声明修正source启动范围，
不放宽评测器或改动注册样本。下方暂停记录与旧“下一步”为历史，不覆盖本次恢复授权。
