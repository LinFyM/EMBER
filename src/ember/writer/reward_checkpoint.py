"""Cycle checkpoints for CFMG median-capped task commitment."""
...
REWARD_CHECKPOINT_SCHEMA = (
    "ember_pi05_v6_lpcp_cfmg_median_capped_task_tangent_commitment_checkpoint_v1"
)
REWARD_DEPLOYMENT_KIND = (
    "v6_lpcp_cfmg_median_capped_task_tangent_commitment_cycle_checkpoint"
)
    ...
    *,
    ...
    if context.is_main:
        ...
        if partial.exists() or final.exists():
            raise WriterModelError("direct-factor checkpoint already exists")
        partial.mkdir()
    barrier(context)
    torch.save(
        {
...
def load_reward_checkpoint(
    *,
    ...
    if (
        ...
        raise WriterModelError("direct-factor checkpoint manifest changed")
    ...
    if (
        ...
        raise WriterModelError("direct-factor checkpoint cursor changed")
