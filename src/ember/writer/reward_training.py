"""Train pre-addressed factor selectors with cross-video success credit."""
...
def _load_tasks(
    *, data_root: Path, base_config: Mapping[str, Any]
    ...
    if len(reward_tasks) != 24 or [task.global_task_id for task in reward_tasks] != [
        ...
        raise WriterModelError("factor-selector run lost train24 task authority")
    ...
    if context.is_main:
        ...
        if runtime_args.resume is None:
            if runtime_args.output_dir.exists() and any(
                ...
                raise WriterModelError("fresh factor-selector output is not empty")
            ...
            raise WriterModelError(
                "factor-selector exact-resume launch contract changed"
            )
        append_jsonl(
...
def _load_factor_selector_models(
    *,
    args: argparse.Namespace,
    context: DistributedContext,
    config: dict[str, Any],
    ...
    if not (cold_start / "writer.safetensors").is_file():
        raise WriterModelError("factor-selector LPCP cold start is missing")
    ...
    if (
        ...
        raise WriterModelError(
            "pre-addressed commitment must train only 16,384 selectors"
        )
    trainable = writer_trainable_contract(writer, policy, lora)
    trainable["object"] = (
        "v6_lpcp_preaddressed_factor_selectors_success_credit_only"
    )
...
def prepare_runtime(
    ...
    if args.mode == "smoke" and context.world_size != 1:
        raise WriterModelError("factor-selector smoke uses one GPU")
    allowed = config["formal_run"]["allowed_world_sizes"]
    if context.world_size not in allowed:
        raise WriterModelError("factor-selector world size is outside 1--6")
    if args.mode == "formal":
        state = git_state(Path(__file__).resolve().parents[3])
        if not git_state_is_clean_pushed_or_frozen_authority(state):
            raise WriterModelError(
                "formal factor-selector training requires clean pushed Git"
            )
    ...
    policy, writer, lora, trainable, optimizer = _load_factor_selector_models(
        args=args,
        context=context,
        config=config,
    ...
    if args.resume is not None:
        ...
        if loaded != start_cycle:
            raise WriterModelError("factor-selector resume cursor changed")
    stop_cycle = (
        1
        if args.mode == "smoke"
    ...
    if args.mode == "formal" and (
        ...
        raise WriterModelError("factor-selector formal stop boundary changed")
    ...
    try:
        ...
        if context.is_main:
            write_json_atomic(
                ...
                {
                    "schema_version": (
                        "ember_pi05_v6_lpcp_preaddressed_factor_selective_"
                        "native_value_"
                        "completion_v1"
                    ),
                    "mode": args.mode,
