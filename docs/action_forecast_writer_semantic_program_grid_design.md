...
状态：2026-07-31 owner确认的下一整体架构设计，尚未实现或训练。
...
保留24个task gradients到macro boundary
确定性消除彼此一阶负冲突
...
主因应转向functional AS surrogate与closed-loop success错位，届时进入独立
short-AS cold-start后的pure-reward RL Writer，而不是追加AS结构补丁。
