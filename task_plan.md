...
## 已完成：固定A900的Core／Procedure交叉诊断（2026-09-18）

1. 同语言、teacher46与固定96初态条件在输出前登记；两donor各自分析并排除自身task复核。
2. 已完成168条功能记录、96个混合LoRA与384条新增闭环；复用288条对角线，全部配对检查通过。
3. CC58、CW34／35、WC56／56、WW38／38。正确P在另一Core下仍带来净18个成功，两个区间为正。
4. 已记录逐task/suite、breadth、R/G/L、交互及分布外等解释边界；21workers退出，原件和复现脚本保留。
   当前支持保留有效Core/P分工及融合，尚未确定新架构或fresh修改；无待恢复实验。

原件：[完整报告](runs/analysis/v52_core_procedure_cross_20260918/report.md)。
