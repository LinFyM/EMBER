...
主讨论在同一turn继续下一项完整方法问题，不等待新的Owner消息，也不需要在每个研究段之间自发Queue。
本段预计30–45分钟：从合法原生图像patch/任务语言和真实帧对应出发，给出一个具体计算如何保留
“哪个对象发生了什么相对关系变化”，再说明这些特征怎样生成作用于自身状态的完整LoRA及获得真实FM信用。
不能重新把角色/过程特征当作已知输入，也不能仅重复§17已假设角色绑定正确后的学习例子。
对照旧Horizon完整H软对应及两端真实Z读取、SemanticPath、VisibleObject与LocalActionGrounded，
判断有没有实质计算差别足以改变近邻失败预测；没有则不以新名称/额外encoder/辅助标签启动。
本段不启动模型、环境、GPU，不另派历史审计，不扩文献，不把原生H称为教师真实动作。
