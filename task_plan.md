...
3. **当前理论任务：**综合findings§62–73与早期正例，区分任务共用能力、视频过程信息、初始化关系变化和共享参数竞争。
   使用既有行为／训练证据明确各解释已支持什么、还缺什么；先形成会产生不同观察的可失败预测。
   不把state-free教师prompt（早期强模型同样使用）当作新实现缺陷，不预设E充分或Compiler唯一失败。
