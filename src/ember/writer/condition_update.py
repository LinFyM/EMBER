...
@dataclass(frozen=True)
class SharedRewardTangentProjectionSummary:
    """Small-dual evidence for one SRTP projection of the final shared write."""

    mixed_constraints: int
    anchor_constraint_rows: int
    anchor_rank: int
    constraint_gram_rank: int
...
def _reward_constraint_values(
    features: torch.Tensor,
    cotangents: torch.Tensor,
    delta: torch.Tensor,
) -> torch.Tensor:
    motion = success_key_constraint_motion(features, delta)
    return (cotangents.to(dtype=torch.float32) * motion).flatten(1).sum(dim=1)


@torch.no_grad()
def shared_reward_tangent_program_projection(
    blind_delta: torch.Tensor,
    correct_features: torch.Tensor,
    reward_cotangents: torch.Tensor,
    mixed_mask: torch.Tensor,
    anchor_features: torch.Tensor,
) -> tuple[torch.Tensor, SharedRewardTangentProjectionSummary]:
    """Project the final SKNC update into all mixed-task reward half-spaces."""

    conditions = correct_features.shape[0] if correct_features.ndim == 2 else 0
    if (
        conditions <= 0
        or reward_cotangents.ndim != 3
        or reward_cotangents.shape[0] != conditions
        or tuple(reward_cotangents.shape[1:]) != tuple(blind_delta.shape[1:])
        or blind_delta.ndim != 3
        or blind_delta.shape[0] != correct_features.shape[1]
        or mixed_mask.shape != (conditions,)
        or mixed_mask.dtype != torch.bool
        or anchor_features.ndim != 2
        or anchor_features.shape[1:] != correct_features.shape[1:]
        or len(
            {
                ...
                reward_cotangents.device,
                mixed_mask.device,
                anchor_features.device,
            }
        )
        != 1
    ):
        raise ConditionUpdateError("invalid shared reward-tangent projection batch")
    if not all(
        bool(torch.isfinite(value).all())
        for value in (
            blind_delta,
        ...
        raise ConditionUpdateError("shared reward-tangent projection is non-finite")

    selected_features = correct_features[mixed_mask].to(dtype=torch.float32)
    selected_cotangents = reward_cotangents[mixed_mask].to(dtype=torch.float32)
    mixed_count = int(mixed_mask.sum())
    if mixed_count and bool(
        (selected_cotangents.flatten(1).square().sum(dim=1) <= 0).any()
    ):
        raise ConditionUpdateError("mixed reward constraint has zero cotangent")

    anchor_rank = 0
    basis64 = correct_features.new_empty(
        (correct_features.shape[1], 0), dtype=torch.float64
    )
    projected_features64 = selected_features.to(dtype=torch.float64)
    if anchor_features.shape[0]:
        anchors64 = anchor_features.to(dtype=torch.float64)
        _, singular_values, vh = torch.linalg.svd(anchors64, full_matrices=False)
        maximum = float(singular_values.max()) if singular_values.numel() else 0.0
        ...
        anchor_rank = int((singular_values > tolerance).sum())
        if anchor_rank <= 0:
            raise ConditionUpdateError("reward projection anchors have zero rank")
        basis64 = vh[:anchor_rank].transpose(0, 1).contiguous()
        projected_features64 = projected_features64 - (
            projected_features64 @ basis64
        ) @ basis64.T

    raw_constraints = _reward_constraint_values(
        selected_features, selected_cotangents, blind_delta
    )
    raw_violations = int((raw_constraints > 0).sum())
    projected = blind_delta
    active_dual_count = 0
    gram_rank = 0
    if mixed_count and raw_violations:
        projected_features = projected_features64.to(dtype=torch.float32)
        cotangent_flat = selected_cotangents.flatten(1)
        previous_tf32 = None
        if blind_delta.is_cuda:
            previous_tf32 = torch.backends.cuda.matmul.allow_tf32
            torch.backends.cuda.matmul.allow_tf32 = False
        try:
            feature_gram = projected_features @ projected_features.transpose(0, 1)
            cotangent_gram = cotangent_flat @ cotangent_flat.transpose(0, 1)
        finally:
            if previous_tf32 is not None:
                torch.backends.cuda.matmul.allow_tf32 = previous_tf32
        gram = (
            feature_gram.to(dtype=torch.float64)
            * cotangent_gram.to(dtype=torch.float64)
        ).cpu().numpy()
        gram = (gram + gram.T) * 0.5
        right = raw_constraints.to(dtype=torch.float64).cpu().numpy()
        eigenvalues, eigenvectors = np.linalg.eigh(gram)
        maximum = float(np.max(np.abs(eigenvalues))) if eigenvalues.size else 0.0
        tolerance = max(1, mixed_count) * np.finfo(np.float64).eps * maximum
        positive = eigenvalues > tolerance
        gram_rank = int(np.count_nonzero(positive))
        if gram_rank <= 0:
            raise ConditionUpdateError("violated reward constraints have zero rank")
        ...
        dual, _ = nnls(square_root, inverse_root @ right)
        active_dual_count = int(np.count_nonzero(dual > 0))
        dual_tensor = torch.from_numpy(dual).to(
            device=blind_delta.device, dtype=torch.float32
        )
        correction_flat = projected_features.transpose(0, 1) @ (
            dual_tensor[:, None] * cotangent_flat
        )
        projected_flat = blind_delta.flatten(1) - correction_flat
        if anchor_rank:
            basis = basis64.to(dtype=torch.float32)
            projected_flat.sub_(basis @ (basis.transpose(0, 1) @ projected_flat))
        projected = projected_flat.reshape_as(blind_delta).contiguous()

    final_constraints = _reward_constraint_values(
        selected_features, selected_cotangents, projected
    )
    if mixed_count:
        constraint_scale = (
            selected_cotangents.flatten(1).square().sum(dim=1).sqrt()
            * success_key_constraint_motion(selected_features, projected)
            .flatten(1)
            .square()
            .sum(dim=1)
            .sqrt()
        ...
        numerical_tolerance = (
            ...
            * constraint_scale.clamp_min(torch.finfo(torch.float32).tiny)
        )
        final_violations = int((final_constraints > numerical_tolerance).sum())
    else:
        final_violations = 0
    correction = blind_delta - projected
    blind_flat = blind_delta.flatten()
    projected_flat = projected.flatten()
    blind_energy = blind_flat.square().sum()
    projected_energy = projected_flat.square().sum()
    inner = torch.dot(blind_flat, projected_flat)
    cosine = inner / (
        blind_energy.sqrt() * projected_energy.sqrt()
    ).clamp_min(torch.finfo(torch.float32).tiny)
    anchor_motion = success_key_constraint_motion(anchor_features, projected).flatten(1)
    summary = SharedRewardTangentProjectionSummary(
        mixed_constraints=mixed_count,
        anchor_constraint_rows=int(anchor_features.shape[0]),
        anchor_rank=anchor_rank,
        constraint_gram_rank=gram_rank,
        raw_violation_count=raw_violations,
        final_violation_count=final_violations,
        raw_constraint_max=(
            float(raw_constraints.max()) if raw_constraints.numel() else 0.0
        ),
        final_constraint_max=(
            float(final_constraints.max()) if final_constraints.numel() else 0.0
        ),
        active_dual_count=active_dual_count,
        projection_changed=not torch.equal(projected, blind_delta),
        correction_rms=_root_mean_square(correction),
        blind_delta_rms=_root_mean_square(blind_delta),
        projected_delta_rms=_root_mean_square(projected),
        projected_to_blind_energy_ratio=float(
            projected_energy / blind_energy.clamp_min(torch.finfo(torch.float32).tiny)
        ),
        blind_projected_inner_product=float(inner),
        blind_projected_cosine=float(cosine),
        predicted_anchor_motion_rms=(
            _root_mean_square(anchor_motion) if anchor_motion.numel() else 0.0
        ),
        predicted_anchor_motion_max_abs=(
            float(anchor_motion.abs().max()) if anchor_motion.numel() else 0.0
        ),
    )
    if not bool(torch.isfinite(projected).all()) or not all(
        math.isfinite(value)
        for value in (
            summary.raw_constraint_max,
            summary.final_constraint_max,
            summary.projected_to_blind_energy_ratio,
            summary.blind_projected_inner_product,
            summary.blind_projected_cosine,
        )
    ):
        raise ConditionUpdateError("shared reward-tangent projection became invalid")
