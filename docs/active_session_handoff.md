...
v6 fresh task-complete 正式首段正在 GPU4–7 运行：
...
2026-07-28 本次文档快照时，macro100 checkpoint 已落盘，训练 tmux 存活；
25/50/75 的 online task-balanced functional loss 依次为
`.130744/.133971/.133841`。这些 loss 只作数值监控，不能替代 closed-loop
rollout。接手时必须读取真实 `metrics.jsonl`、checkpoint 和 validation 文件，
不要把上述数字当作当前末步。
...
stage extension。macro200 进程退出后才可验证、commit、push、合入 main。
...
1. 等正式进程自然停在 macro200，核对 tmux 退出码、run summary、metrics 行数、
   25..200 全部 checkpoint、四 rank state、task/video/query 计数和
   online validation；不得重复启动或覆盖输出。
2. 合入已验证的清理/resume 窄修正；main 与 origin/main 必须再次一致且 clean。
3. 在 GPU4–7 做均匀 fixed correct400 screen，默认 checkpoint 为
   macro50/100/150/200；每张卡只加载一个 checkpoint。若曲线峰值需要更密集
   定位，再利用每 25 保存的点补测，不预先全扫。
4. evaluator 使用 6 个 Writer generators + 6 个 persistent policy workers，
   teacher video 做 50 条无放回随机匹配，所有 worker 全局 long-first；不得用
   静态 task/GPU 分配。
5. 以 closed-loop absolute、paired 多 task 贡献和曲线选择 observed-best。
...
6. 除非 macro0→200 的 closed-loop absolute 明确下降，否则 exact-resume 同一
   recipe 到 macro400。平台、轻微波动或 online loss 上升都不是跳过第二段的
   理由；第三段必须重新由真实曲线决定。
7. v6 通过 absolute 与视频特异性门后，fresh 训练 corrected mixed-task
...
8. 不自动进入 final-32、test task-local RL 或 joint oracle。
...
`/data/ymdai` 的 operator hard cap 是 500GB。正式首段启动前个人占用为
`401,720,096,428` bytes，预计本段新增约 1–1.5GB；任何新正式 run、批量
rollout cache 或 checkpoint 扩展前都要重新测现场和峰值空间。
