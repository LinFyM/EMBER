"""One-time train24/demo16--41 spatial labels; never read action values.

Geometry, OOI, visibility and motion definitions are copied from 05fe7ebe.
Retire this entrypoint after the complete 624-episode labels are verified.
"""
...
# Spatial construction is CPU-only, including the offscreen renderer.
...
def main():
    ...
    if args.validate_only:
        print(json.dumps(dict(schema=SCHEMA, tasks=config["data"]["task_ids"],
                              demos=list(range(16, 42)), episodes=624, environment_steps=0)))
        return
    args.output.mkdir(parents=True, exist_ok=False)
    write_json(args.output / "registration.json", dict(schema=SCHEMA, tasks=config["data"]["task_ids"],
               demos=list(range(16, 42)), design="docs/native_correction_writer_design.md", workers=args.workers,
               gradients=False, environment_steps=0, source_frame_stride=5, include_last_frame=True))
    jobs = [(str(args.asset_root), str(args.output), row, demo) for row in tasks for demo in range(16, 42)]
    rows = []
